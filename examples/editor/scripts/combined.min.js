...
"?method=list&depth=0",dataType:"json",error:k,success:function(a){i();j.listing=a.items;(!j.filename||j.filename.substr(-1)=="/")&&f()}})}function d(a){j.filename=a;a==""||a.substr(-1)=="/"?f():$.ajax({url:"/mirror/"+j.app_id+"/"+j.filename,dataType:"text",error:k,success:function(a){i();$("#content").empty();j.editor.loadFile(j.filename,a);j.editor.adjustHeight("shrink");j.editor.view("show");g("Loaded file: "+j.filename)}})}function h(){var a=$("iframe.upload")[0];a&&(a=a.contentWindow.document.body.innerHTML.replace(/<\/?[^>]+>\s*/gi,
...
a);j.editor.view("show")}else j.editor=j[$("#editor").val()]}function m(){o||j.editor.view()}function n(){var a=$("#ace"),c=$("#textarea");a.css("height",window.innerHeight-43+"px");a.css("width",window.innerWidth+"px");c.css("height",window.innerHeight-43+"px");c.css("width",window.innerWidth-15+"px")}var j={},o;b.extend({onReady:function(){var a=namespace.lookup("com.pageforest.client");j.client=new a.Client(b);j.client.saveInterval=0;j.client.state="clean";j.hash="###";j.app_id="";j.filename="";
setInterval(e,200);setInterval(h,1E3);setInterval(m,5E3);j.ace=namespace.lookup("com.pageforest.editor.ace");j.ace.createEditor();j.ace.view("hide");j.textarea=namespace.lookup("com.pageforest.editor.textarea");j.textarea.view("hide");var a=j,c;c=!1;/Android|iPad|iPod|iPhone/.test(navigator.userAgent)&&(c=!0);c?($("#editor").val("textarea"),c=j.textarea):($("#editor").val("ace"),c=j.ace);a.editor=c;$(window).bind("resize",n);$("#editor").change(l)},onSave:function(){$.ajax({type:"PUT",url:"/mirror/"+
j.app_id+"/"+j.filename,data:j.editor.getData(),dataType:"text",success:function(a){g(a)},error:k})},onUserChange:function(c){console.log("onUserChange("+c+");");c==void 0?($("#username").text("anonymous"),$("#signin").text("Sign in").attr("href","http://www.pageforest.com/sign-in/editor")):($("#username").text(c),$("#signin").text("Sign out").attr("href","http://www.pageforest.com/sign-out/editor"));j.app_id||a()},onSignInOut:function(){j.client.username!=void 0?j.client.signOut():j.client.signIn()},
appListing:j.appListing,listing:j.listing,app_id:j.app_id,filename:j.filename,editor:j.editor,hash:j.hash,client:j.client})});namespace.module("com.pageforest.editor.textarea",function(b){function g(b){console.log("adjustScroll event: ",b)}var k=!1;b.extend({createEditor:function(){},loadFile:function(b,f){var a=$('<textarea id="code" spellcheck="false" autocorrect="off" ></textarea>');$("#textarea").empty().append(a);a.val(f).focus();a.bind("touchstart",g)},adjustHeight:function(b){console.log("adjustHeight() within textarea");for(var f=$("#code"),a=f.attr("scrollHeight"),c=f.attr("offsetHeight");b&&c&&a==c;)f.css("height",
