...
re_local_blob = re.compile(r'docs/(' + DOC_ID_REGEX + ')_blobs/(.*)$')
re_blob = re.compile(r'docs/(' + DOC_ID_REGEX + ')/(.*)$')


def normalize_local_path(path):
    """
    Converts 'docs\docid_blobs\path' to 'docs/docid/path'
    """
    path = path.replace(os.path.sep, '/')
    match = re_local_blob.match(path)
    if match:
        path = "docs/%s/%s" % (match.group(1), match.group(2))
    return path
...
def get_local_path(path):
    ...
    if match:
        path = BLOB_DIR % (match.group(1), match.group(2))
    return path
