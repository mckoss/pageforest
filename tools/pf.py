...
BLOB_RE = re.compile(r'docs/(' + DOC_ID_REGEX + ')_blobs/(.*)$')
...
def is_doc_filename(filename):
    return is_doc_path(filename) and filename.count('/') == 1


def is_blob_filename(filename):
    return is_doc_path(filename) and filename.count('/') >= 1
...
def list_remote_path(path):
    ...
    while True:
        ...
        if options.verbose:
            print "Listing %s (%s files)" % (url, intcomma(count))
...
def put_command(args):
    ...
    for path in paths:
        if path == META_FILENAME:
            continue
        if (not options.docs) is is_doc_path(path):
            continue
