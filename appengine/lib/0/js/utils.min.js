...
namespace.lookup('org.startpad.cookies').define(function(ns){var base=namespace.lookup('org.startpad.base');ns.extend({setCookie:function(name,value,days,path){var expires='';if(days){var date=new Date();date.setTime(date.getTime()+days*24*60*60*1000);expires='; expires='+date.toGMTString();}
path='; path='+(path||'/');document.cookie=encodeURIComponent(name)+'='+encodeURIComponent(value)
+expires+path;},getCookie:function(name){return ns.getCookies()[name];},getCookies:function(name){var st=document.cookie;var rgPairs=st.split(";");var obj={};for(var i=0;i<rgPairs.length;i++){rgPairs[i]=base.strip(rgPairs[i]);var rgC=rgPairs[i].split("=");var val=decodeURIComponent(rgC[1]);var rg=val.match('^"(.*)"$');if(rg)
...
namespace.lookup('com.pageforest.client').defineOnce(function(ns){var cookies=namespace.lookup('org.startpad.cookies');var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var pollInterval=500;var discardMessage="You will lose your document changes if you continue.";function Client(app){this.app=app;this.appHost=location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.lastHash='';this.state=Client.states.clean;this.username=undefined;this.fLogging=false;setInterval(this.poll.fnMethod(this),pollInterval);$(window).bind('beforeunload',this.beforeUnload.fnMethod(this));}
Client.states=new base.Enum('clean','dirty','loading','saving');Client.methods({load:function(docid){if(this.state==Client.states.dirty){if(!this.confirmDiscard()){return;}
this.state=Client.states.clean;}
this.stateSave=this.state;this.state=Client.states.loading;this.log("loading: "+docid);$.ajax({dataType:'json',url:this.getDocURL(docid),error:this.errorHandler.fnMethod(this),success:function(document,textStatus,xmlhttp){this.setCleanDoc(docid);this.app.onLoad(document);}.fnMethod(this)});},getDocURL:function(docid){if(docid==undefined){docid=this.docid;}
return'http://'+this.appHost+'/docs/'+docid+'/';},save:function(json,docid){if(this.username==undefined){this.errorReport('no_username',"You must sign in to save.");}
if(json==undefined){json=this.app.onSave();}
if(docid==undefined){docid=this.docid;}
if(docid==undefined){docid=this.username+'-'+json.title;docid=format.slugify(docid);}
this.stateSave=this.state;this.state=Client.states.saving;if(!json.readers){json.readers=['public'];}
var data=JSON.stringify(json);this.log('saving: '+docid,json);$.ajax({type:'PUT',url:'/docs/'+docid,data:data,error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},setCleanDoc:function(docid){this.docid=docid;this.state=Client.states.clean;location.hash=this.docid;this.lastHash=location.hash;},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,obj){if(this.fLogging){if(obj!=undefined){console.log(message,obj);}
else{console.log(message);}}},errorHandler:function(xmlhttp,textStatus,errorThrown){this.state=this.stateSave;var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',xmlhttp);this.errorReport(code,message);},errorReport:function(status,message){if(this.app.error){this.app.error(status,message);}
else{alert(status+': '+message);}},confirmDiscard:function(){if(this.app.confirmDiscard){return this.app.confirmDiscard();}
return confirm(discardMessage);},makeDirty:function(fDirty){if(fDirty==undefined){fDirty=true;}
this.state=fDirty?Client.states.dirty:Client.states.clean;},beforeUnload:function(evt){if(this.state!=Client.states.clean){evt.returnValue="You will lose your changes if you leave "+"the document without saving.";return evt.returnValue;}},poll:function(){if(this.lastHash!=location.hash){this.lastHash=location.hash;this.load(location.hash.substr(1));}
...
if(this.app.onUserChanged&&usernameLast!=this.username){this.log('found user: '+this.username);this.app.onUserChanged(this.username||'anonymous');}},signIn:function(){window.open('http://'+this.wwwHost+"/sign-in/"+
