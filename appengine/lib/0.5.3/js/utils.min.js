...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),beforeSend:function(xhr){if(this.sessionKey){xhr.setRequestHeader("Authorization",'PFSK1 '+this.sessionKey);}},success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},detach:function(){this.setCleanDoc();this.setDirty();},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=JSON.stringify(this.app.getData());if(preserveHash){this.lastHash='';return;}
...
return'http://docs.'+this.appHost+'/'+docid+'/';},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,obj){if(this.fLogging){if(obj!=undefined){console.log(message,obj);}
