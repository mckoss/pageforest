...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),{'obj':json});$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),beforeSend:function(xhr){if(this.sessionKey){xhr.setRequestHeader("Authorization",'PFSK1 '+this.sessionKey);}}.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},detach:function(){this.setCleanDoc();this.setDirty();},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=JSON.stringify(this.app.getDoc());if(preserveHash){this.lastHash='';return;}
...
return'http://'+this.appHost+'/docs/'+docid+'/';},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,options){if(options==undefined){options={};}
if(options.level==undefined){options.level='log';}
if(this.fLogging){if(options.obj!=undefined){console[options.level](message,options.obj);}else{console[options.level](message);}}},errorHandler:function(xmlhttp,textStatus,errorThrown){this.changeState(this.stateSave);var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',{'obj':xmlhttp});this.errorReport(code,message);},errorReport:function(status,message){if(this.app.onError){this.app.onError(status,message);}
...
this.checkUsername();this.checkDoc();},checkUsername:function(){var sessionKeyExists=cookies.getCookie('sessionkey_exists');var usernameLast=this.username;if(sessionKeyExists!=undefined){if(!this.sessionKey){$.ajax({dataType:'text',url:'http://'+this.appHost+'/auth/get-session/',error:this.errorHandler.fnMethod(this),success:function(sessionKey,textStatus,xmlhttp){this.sessionKey=sessionKey;this.username=sessionKey.split('|')[1];this.log('signed in as '+this.username);if(this.app.onUserChange&&usernameLast!=this.username){this.app.onUserChange(this.username);}}.fnMethod(this)});}}
else{this.username=undefined;this.sessionKey=undefined;if(this.app.onUserChange&&usernameLast!=this.username){this.app.onUserChange(this.username);}}},checkDoc:function(){if(this.saveInterval==0){return;}
...
this.appid+'/','_blank');},signOut:function(){cookies.setCookie('sessionkey_exists','expired',-1);$.ajax({dataType:'text',url:'http://'+this.appHost+'/auth/set-session/expired/',error:this.errorHandler.fnMethod(this),success:function(sessionKey,textStatus,xmlhttp){this.log("sessionkey deleted");}.fnMethod(this)});}});ns.extend({Client:Client});});
