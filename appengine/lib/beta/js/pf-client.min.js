...
break;case'TEXTAREA':field.elt.value=value;break;default:dom.setText(field.elt,value);break;}}}},setErrors:function(errors){for(var i=0;i<this.fields.length;i++){var field=this.fields[i];if(!field.error){continue;}
dom.setText(field.error,errors[field.name]||'');}},setFocus:function(){var field;this.bindFields();if(this.focus){field=this.getField(this.focus);if(field){field.elt.focus();field.elt.select();}}},getValues:function(){var values={};this.bindFields();for(var i=0;i<this.fields.length;i++){var field=this.fields[i];if(field.elt==undefined){continue;}
...
var rcAppBox=dom.getRect($('#pfAppBarBox')[0]);dom.slide(this.appPanel,[rcAppBox[2],0],animation,fnCallback);},showError:function(message){if(this.errorPanel==undefined){return;}
