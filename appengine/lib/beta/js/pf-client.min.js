...
else{jQuery('#pfSave').removeClass('disabled');}}},beforeUnload:function(evt){evt=evt||window.event;if(this.state!='clean'){evt.returnValue=unloadMessage;return unloadMessage;}},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,options){if(!this.fLogging){return;}
if(options==undefined){options={};}
if(!options.hasOwnProperty('level')){options.level='log';}
if(options.once){if(this.logged[message]){return;}
this.logged[message]=true;}
...
this.onError(code,message);},onError:function(status,message){this.log(message+' ('+status+')');this.showError(message);if(this.app.onError){this.app.onError(status,message);}},onInfo:function(code,message){this.log(message+' ('+code+')');if(this.app.onInfo){this.app.onInfo(code,message);}},getDocid:function(){var hash;if(this.oneDocPerUser){return this.username;}
