...
break;case'TEXTAREA':field.elt.value=value;break;default:dom.setText(field.elt,value);break;}}}},setErrors:function(errors){this.focus=undefined;for(var i=0;i<this.fields.length;i++){var field=this.fields[i];if(!field.error){continue;}
var error=errors[field.name]||'';dom.setText(field.error,error);if(error&&!this.focus){this.focus=field.name;}}
this.setFocus();},setFocus:function(){var field;this.bindFields();if(this.focus){field=this.getField(this.focus);if(field){field.elt.focus();field.elt.select();}}},getValues:function(){var values={};this.bindFields();for(var i=0;i<this.fields.length;i++){var field=this.fields[i];if(field.elt==undefined){continue;}
