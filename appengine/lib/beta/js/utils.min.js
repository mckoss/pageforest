...
if(field.onClick!=undefined){dom.bind(field.elt,'click',onClick);}
if(self.focus==undefined&&(field.elt.tagName=='INPUT'||field.elt.tagName=='TEXTAREA')){self.focus=field.name;}});this.bound=true;},getField:function(name){for(var i=0;i<this.fields.length;i++){if(this.fields[i].name==name){return this.fields[i];}}
...
break;case'TEXTAREA':field.elt.value=value;break;default:dom.setText(field.elt,value);break;}}}},setFocus:function(){var field;if(this.focus){field=this.getField(this.focus);if(field){field.elt.focus();field.elt.select();}}},getValues:function(){var values={};this.bindFields();for(var i=0;i<this.fields.length;i++){var field=this.fields[i];if(field.elt==undefined){continue;}
...
var data=JSON.stringify(json);this.log('saving document: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:function(xmlhttp,textStatus,errorThrown){this.errorHandler(xmlhttp,textStatus,errorThrown);fn(false);},success:function(){fn(true);}});},putBlob:function(docid,blobid,data,options,fn){fn=fn||function(){};options=options||{};if(docid==undefined){this.onError('unsaved_document',docUnsavedMessage);fn(false);return;}
...
var rcAppBox=dom.getRect($('#pfAppBarBox')[0]);dom.slide(this.appPanel,vector.lr(rcAppBox),animation,fnCallback);},showError:function(message){var rcAppBox=dom.getRect($('#pfAppBarBox')[0]);if(this.errorPanel==undefined){return;}
