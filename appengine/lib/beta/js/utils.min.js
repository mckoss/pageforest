...
return num;},ptInRect:function(pt,rc){return ns.numInRange(pt[ns.x],rc[ns.x],rc[ns.x2])&&ns.numInRange(pt[ns.y],rc[ns.y],rc[ns.y2]);},ptClipToRect:function(pt,rc){return[ns.clipToRange(pt[ns.x],rc[ns.x],rc[ns.x2]),ns.clipToRange(pt[ns.y],rc[ns.y],rc[ns.y2])];},rcClipToRect:function(rc,rcClip){return ns.append(ns.ptClipToRect(ns.ul(rc),rcClip),ns.ptClipToRect(ns.lr(rc),rcClip));},rcExpand:function(rc,ptSize){var rcExp=ns.append(ns.sub(ns.ul(rc),ptSize),ns.add(ns.lr(rc),ptSize));var ptCenter=ns.ptCenter(rc);if(rcExp[ns.x]>rcExp[ns.x2]){rcExp[ns.x]=rcExp[ns.x2]=ptCenter[ns.x];}
if(rcExp[ns.y]>rcExp[ns.y2]){rcExp[ns.y]=rcExp[ns.y2]=ptCenter[ns.y];}
return rcExp;},keepInRect:function(rcIn,rcBound){var ptFixSize=ns.max([0,0],ns.sub(ns.size(rcIn),ns.size(rcBound)));rcIn[ns.x2]-=ptFixSize[ns.x];rcIn[ns.y2]-=ptFixSize[ns.y];var dx=0;var dy=0;dx=Math.max(0,rcBound[ns.x]-rcIn[ns.x]);dy=Math.max(0,rcBound[ns.y]-rcIn[ns.y]);if(dx==0){dx=Math.min(0,rcBound[ns.x2]-rcIn[ns.x2]);}
...
return[ptMin[0],ptMin[1],ptMax[0],ptMax[1]];}});ns.copy=ns.append;});
/* Begin file: dom.js */
