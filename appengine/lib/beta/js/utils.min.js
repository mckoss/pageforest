...
Function.prototype.methods=function(obj){extendObject(this.prototype,obj);};Function.methods({fnMethod:function(obj){var _fn=this;return function(){return _fn.apply(obj,arguments);};},fnArgs:function(){var _fn=this;var _args=copyArray(arguments);return function(){var args=copyArray(arguments).concat(_args);return _fn.apply(this,args);};},fnWrap:function(fn){var _fn=this;return function(){return _fn(this,fn,arguments);};},decorate:function(decorator){var fn=this;return function(){return decorator.call(this,fn,arguments);};}});Namespace.methods({define:function(closure){this._isDefined=true;this._closure=closure;if(namespaceT.logLevel<=1){console.info("Namespace '"+this._path+"' defined.");}
...
namespace.lookup('org.startpad.format').defineOnce(function(ns){var util=namespace.util;var base=namespace.lookup('org.startpad.base');var comma=',';function fixedDigits(value,digits,fSign){var s="";var fNeg=(value<0);if(digits==undefined){digits=0;}
if(fNeg){value=-value;}
...
if(value==undefined){console.log("format error: "+keys.slice(0,i+1).join('.'));return"";}}
...
String.prototype.format=function(){var args=util.copyArray(arguments);args.unshift(reFormat);return formatImpl.apply(this,args);};ns.extend({'fixedDigits':fixedDigits,'thousands':thousands,'slugify':slugify,'escapeHTML':escapeHTML,'format':format,'replaceKeys':format,'replaceString':replaceString,'base64ToString':base64ToString,'canvasToPNG':canvasToPNG,'dateFromISO':dateFromISO,'isoFromDate':isoFromDate,'setTimezone':setTimezone,'decodeClass':decodeClass,'shortDate':shortDate,'wordList':wordList,'arrayFromWordList':arrayFromWordList,'repeat':repeat});});
...
namespace.lookup('org.startpad.dialog').defineOnce(function(ns){var util=namespace.util;var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var dom=namespace.lookup('org.startpad.dom');var defaultPatterns={title:{useRow:'spanRow',content:'<h1>{title}</h1>'},text:{content:'<input id="{id}" type="text"/>'},password:{content:'<input id="{id}" type="password"/>'},checkbox:{label:'',content:'<label class="checkbox" for="{id}">'+'<input id="{id}" type="checkbox"/>&nbsp;{label}</label>'},note:{content:'<textarea id="{id}" rows="{rows}"></textarea>'},message:{useRow:'spanRow',content:'<div class="message" id="{id}"></div>'},value:{label:'<label class="left">{label}:</label>',content:'<div class="value" id="{id}"></div>'},button:{content:'<input id="{id}" type="button" value="{label}"/>'},invalid:{useRow:'spanRow',content:'<span class="error">***missing field type: {type}***</span>'}};var defaultFieldOptions={note:{rows:5}};var styles={div:{label:'<label class="left" for="{id}">{label}:</label>',content:'<input id="{id}" type="text"/>',spanRow:'{content}\n',row:"{label}{content}\n",post:'<div style="clear: both;"></div>\n'},table:{pre:"<table>\n",label:'<label class="left" for="{id}">{label}:</label>',content:'<input id="{id}" type="text"/>',spanRow:"<tr><td columns=2>{content}</td></tr>",row:"<tr><th>{label}</th><td>{content}</td></tr>\n",post:"</table>\n"}};var sDialog='<div class="{dialogClass}" id="{id}">\n'+'{pre}\n{content}\n{post}\n'+'</div>';var cDialogs=0;function Dialog(options){cDialogs++;this.dialogClass='SP_Dialog';this.prefix='SP'+cDialogs+'_';this.bound=false;this.lastValues={};this.patterns=defaultPatterns;this.fieldOptions=defaultFieldOptions;this.style=styles.div;util.extendObject(this,options);util.extendObject(this,this.style);}
Dialog.methods({html:function(){var self=this;var stb=new base.StBuf();this.id=this.prefix+'dialog';base.forEach(this.fields,function(field,i){field.id=self.prefix+i;if(field.type==undefined){field.type='text';}
...
field.name.slice(1);}
var fieldPatterns=base.extendIfMissing({},self.patterns[field.type],base.project(self.style,['label','content']));var row={label:fieldPatterns.label.format(field),content:fieldPatterns.content.format(field)};var rowPattern=self[fieldPatterns.useRow]||self.row;stb.append(rowPattern.format(row));});this.content=stb.toString();return sDialog.format(this);},bindFields:function(){if(this.bound){return;}
...
if(field.onClick!=undefined){dom.bind(field.elt,'click',function(evt){field.onClick(evt,field);});}
...
this.setValues(initialValues);},getField:function(name){for(var i=0;i<this.fields.length;i++){if(this.fields[i].name==name){return this.fields[i];}}
...
this.bindFields();var field=this.getField(name);switch(field.elt.tagName){case'INPUT':case'TEXTAREA':field.elt.disabled=!enabled;break;case'DIV':field.elt.style.display=enabled?'block':'none';break;default:throw new Error("Field "+name+" is not a form field.");}}});ns.extend({'Dialog':Dialog,'styles':styles});});
...
var data=jsonToString(json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getDoc:function(docid,fnSuccess){if(!this.validateArgs('getDoc',docid,undefined,undefined,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};$.ajax({dataType:'json',url:this.getDocURL(docid),error:this.errorHandler,success:function(doc,textStatus,xmlhttp){fnSuccess(doc,textStatus,xmlhttp);}});},deleteDoc:function(docid,fnSuccess){if(!this.validateArgs('deleteDoc',docid,undefined,undefined,undefined,fnSuccess)){return;}
...
$.ajax({type:'PUT',url:url.toString(),data:data,dataType:'json',processData:false,error:this.errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},push:function(docid,blobid,json,options,fnSuccess){if(!this.validateArgs('push',docid,blobid,json,options,fnSuccess)){return;}
...
$.ajax({type:'PUT',url:url.toString(),data:json,dataType:'json',processData:false,error:this.errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getBlob:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('getBlob',docid,blobid,undefined,options,fnSuccess)){return;}
...
$.ajax({type:type,url:url.toString(),dataType:options.dataType||'json',error:this.errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},slice:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('slice',docid,blobid,undefined,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};var url=new URL(this.getDocURL(docid,blobid));url.push('method','slice');url.push('start',options.start);url.push('end',options.end);url.push('wait',options.wait);url.push('etag',options.etag);$.ajax({url:url.toString(),dataType:'json',error:this.errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},deleteBlob:function(docid,blobid,fnSuccess){if(!this.validateArgs('deleteBlob',docid,blobid,undefined,undefined,fnSuccess)){return;}
...
url.push('transfer-encoding',options.encoding);$.ajax({url:url.toString(),dataType:'json',error:this.errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});}});ns.extend({'Storage':Storage,'jsonToString':jsonToString,'getEtag':getEtag});});
/* Begin file: client.js */
namespace.lookup('com.pageforest.client').defineOnce(function(ns){var storage=namespace.lookup('com.pageforest.storage');var cookies=namespace.lookup('org.startpad.cookies');var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var dom=namespace.lookup('org.startpad.dom');var dialog=namespace.lookup('org.startpad.dialog');var vector=namespace.lookup('org.startpad.vector');var random=namespace.lookup('org.startpad.random');ns.pollInterval=1000;var discardMessage="You will lose your document changes if you continue.";var jQueryMessage="jQuery must be installed to use this library.";var unloadMessage="You will lose your changes if you leave "+"the document without saving.";var noSetDocMessage="This app does not have a setDoc method "+"and so cannot be loaded.";var noGetDocMessage="This app does not have a getDoc method "+"and so cannot be saved.";var noAppMessage="Warning: no app object provided - "+"only direct storage api's can be used.";var autoLoadError="Not autoloading: ";var docProps=['title','docid','tags','owner','readers','writers','created','modified'];function Client(app){if(app==undefined){this.log(noAppMessage,{level:'warn'});app={};}
this.app=app;this.errorHandler=this.errorHandler.fnMethod(this);this.storage=new storage.Storage(this);this.meta={};this.metaDoc={};this.metaDialog={};if(typeof $!='function'||$!=jQuery){this.onError('jQuery_required',jQueryMessage);return;}
this.appHost=window.location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.state='init';this.username=undefined;this.fLogging=true;this.logged={};this.lastHash='';this.fFirstPoll=true;this.uid=random.randomString(20);this.saveInterval=60;this.autoLoad=false;if(typeof app.getDoc=='function'){this.emptyDoc=app.getDoc();}
setInterval(this.poll.fnMethod(this),ns.pollInterval);window.onbeforeunload=this.beforeUnload.fnMethod(this);}
...
this.stateSave=this.state;this.docid=docid;this.changeState('loading');var self=this;this.storage.getDoc(docid,function(doc){if(doc.doc_id==undefined){doc.doc_id=docid;}
self.setDoc(doc);});},save:function(json,docid){if(this.isSaved()){return;}
if(json==undefined){json=this.getDoc();}
docid=this.ensureDocid(docid||this.docid||json.docid);this.stateSave=this.state;this.changeState('saving');var self=this;this.storage.putDoc(docid,json,function(result){self.onSaveSuccess(result);});},ensureDocid:function(docid){if(docid){return docid;}
return format.slugify([this.username,base.randomInt(10000)].join(' '));},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner','sha1']));this.setCleanDoc(result.docid);this.setAppPanelValues(this.meta);if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=typeof this.app.getDoc=='function'&&this.app.getDoc();if(typeof doc!='object'){this.log(noGetDocMessage,{level:'warn',once:true});doc={};}
base.extendIfMissing(doc,{'title':document.title});var fDoc=base.extendIfChanged(this.meta,this.metaDoc,base.project(doc,docProps));base.extendIfChanged(this.meta,this.metaDialog,this.getAppPanelValues());base.extendObject(doc,this.meta);this.setAppPanelValues(this.meta);return doc;},setDoc:function(doc){this.meta=base.project(doc,docProps);this.setAppPanelValues(this.meta);this.app.setDoc(doc);this.setCleanDoc(doc.doc_id);},onAutoLoad:function(message){if(!this.autoLoad||message.key!=this.docid+'/'||message.data.modified.isoformat==this.meta.modified.isoformat){this.log(autoLoadError+message.key);return;}
this.load(this.docid);},setCleanDoc:function(docid,preserveHash){this.docid=this.meta.docid=docid;this.changeState('clean');this.lastJSON=storage.jsonToString(this.getDoc());if(this.autoLoad&&this.docid!=undefined){if(!this.storage.hasSubscription(this.docid)){this.storage.subscribe(this.docid,undefined,{exclusive:true},this.onAutoLoad.fnMethod(this));}}
...
else{$('#pfSave').removeClass('disabled');}}},beforeUnload:function(evt){evt=evt||window.event;if(this.state!='clean'){evt.returnValue=unloadMessage;return unloadMessage;}},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,options){if(!this.fLogging){return;}
...
if(options.hasOwnProperty('obj')){console[options.level](message,options.obj);}else{console[options.level](message);}},errorHandler:function(xmlhttp,textStatus,errorThrown){if(this.state=='loading'&&this.emptyDoc){this.app.setDoc(this.emptyDoc);}
if(this.stateSave){this.changeState(this.stateSave);this.stateSave=undefined;}
var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.responseText;if(message.length>100){message=xmlhttp.statusText;}
this.onError(code,message);},onError:function(status,message){this.log(message+' ('+status+')');this.showError(message);if(this.app.onError){this.app.onError(status,message);}},onInfo:function(code,message){this.log(message+' ('+code+')');if(this.app.onInfo){this.app.onInfo(code,message);}},poll:function(){if(this.state=='init'){if(this.getDoc){this.setCleanDoc(undefined,true);}}
...
$('#pfSignIn').text(isSignedIn?'Sign Out':'Sign In');}},addAppBar:function(){var htmlAppBar='<div id="pfAppBarBox">'+'<div class="pfLeft"></div>'+'<div class="pfCenter">'+'{welcome}'+'<span class="pfLink" id="pfUsername"></span>'+'<span class="pfLink" id="pfSignIn">Sign In</span>'+'<span class="pfLink" id="pfSave">Save</span>'+'<div class="expander collapsed" id="pfMore"></div>'+'{logo}'+'</div>'+'<div class="pfRight"></div>'+'</div>';var objFill;if(screen.width>=640){objFill={welcome:'<span id="pfWelcome">Welcome,</span>',logo:'<div id="pfLogo"></div>'};}else{objFill={welcome:'',logo:''};}
htmlAppBar=format.replaceKeys(htmlAppBar,objFill);this.appBar=document.getElementById('pfAppBar');if(!this.appBar){document.body.style.marginTop="39px";this.appBar=document.createElement('div');this.appBar.setAttribute('id','pfAppBar');document.body.appendChild(this.appBar);}
...
this.appDialog.setValues(values);this.appDialog.enableField('docid',this.docid==undefined);this.appDialog.enableField('copy',this.docid!=undefined);},getAppPanelValues:function(){if(this.appPanel==undefined||!$(this.appPanel).is(':visible')){return{};}
