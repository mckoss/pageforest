...
this.appHost=window.location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.state='init';this.username=undefined;this.fLogging=true;this.logged={};this.lastDocid=undefined;this.fFirstPoll=true;this.uid=random.randomString(20);this.saveInterval=60;this.autoLoad=false;if(typeof app.getDoc=='function'){this.emptyDoc=app.getDoc();}
...
return format.slugify([this.username,base.randomInt(10000)].join(' '));},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner','sha1']));this.setCleanDoc(result.docid||this.docid||this.meta.docid);this.setAppPanelValues(this.meta);if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=typeof this.app.getDoc=='function'&&this.app.getDoc();if(typeof doc!='object'){this.log(noGetDocMessage,{level:'warn',once:true});doc={};}
...
if(preserveDocid){this.lastDocid=undefined;return;}
...
this.onError(code,message);},onError:function(status,message){this.log(message+' ('+status+')');this.showError(message);if(this.app.onError){this.app.onError(status,message);}},onInfo:function(code,message){this.log(message+' ('+code+')');if(this.app.onInfo){this.app.onInfo(code,message);}},getDocid:function(){var hash;if(this.app.getDocid){return this.app.getDocid();}
hash=window.location.hash.substr(1);return hash==''?undefined:hash;},setDocid:function(docid){this.lastDocid=docid;if(this.app.setDocid){return this.app.setDocid(docid);}
