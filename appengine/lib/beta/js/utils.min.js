...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),{'obj':json});$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,beforeSend:function(xhr){xhr.setRequestHeader("Content-Type","image/png; charset=binary");},error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},saveBlob:function(blobKey,data,contentType){if(this.docid==undefined){this.errorReport('unsaved_document',docUnsavedMessage);return;}
if(contentType==undefined){contentType=typeof data=='object'?'application/json':'text';}
this.log('saving blob: '+blobKey+' ('+data.length+')');$.ajax({type:'PUT',url:this.getDocURL()+blobKey,data:data,dataType:'text',contentType:contentType,processData:false,error:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',{'obj':xmlhttp});this.errorReport(code,message);}.fnMethod(this),success:function(data){this.log('saved blob: '+blobKey);if(this.app.onBlobSaveSuccess){this.app.onBlobSaveSuccess();}}.fnMethod(this)});},detach:function(){this.setCleanDoc();this.setDirty();},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=JSON.stringify(this.app.getDoc());if(preserveHash){this.lastHash='';return;}
