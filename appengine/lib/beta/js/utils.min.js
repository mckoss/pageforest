...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),beforeSend:function(xhr){if(this.sessionKey){xhr.setRequestHeader("Authorization",'PFSK1 '+this.sessionKey);}}.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},detach:function(){this.setCleanDoc();this.setDirty();},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=JSON.stringify(this.app.getDoc());if(preserveHash){this.lastHash='';return;}
