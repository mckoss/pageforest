...
if(json.blob==undefined){this.onError('missing_blob',blobMessage);return;}
...
var data=this.jsonToString(json);this.log('saving: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){result.docid=docid;result.owner=json.owner||this.username;this.onSaveSuccess(result);}.fnMethod(this)});},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner']));this.setCleanDoc(result.docid);this.setAppPanelValues(this.meta);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=this.app.getDoc();if(typeof doc!='object'){doc={};}
...
return doc;},setDoc:function(doc){this.meta=base.project(doc,docProps);this.setAppPanelValues(this.meta);this.app.setDoc(doc);},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=this.jsonToString(this.getDoc());if(preserveHash){this.lastHash='';return;}
...
var json=this.jsonToString(this.getDoc());if(json!=this.lastJSON){this.setDirty();}},confirmDiscard:function(){if(this.app.confirmDiscard){return this.app.confirmDiscard();}
...
var data=this.jsonToString(json);this.log('saving document: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:function(xmlhttp,textStatus,errorThrown){this.errorHandler(xmlhttp,textStatus,errorThrown);fn(false);},success:function(){fn(true);}});},jsonToString:function(json){var s;var badProperty;function mapper(key,value){if(badProperty){return value;}
...
return s;},putBlob:function(docid,blobid,data,options,fn){fn=fn||function(){};options=options||{};if(docid==undefined){this.onError('unsaved_document',docUnsavedMessage);fn(false);return;}
...
if(typeof data!="string"){data=this.jsonToString(data);}
