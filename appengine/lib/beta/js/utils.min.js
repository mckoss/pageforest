...
namespace.lookup('com.pageforest.storage').defineOnce(function(ns){var base=namespace.lookup('org.startpad.base');var util=namespace.util;var format=namespace.lookup('org.startpad.format');var loader=namespace.lookup('org.startpad.loader');var errorMessages={no_username:"You must sign in to save a document.",bad_options:"API Call invalid",bad_callback:"API Call invalid",slice_range:"Invalid slice range (start or end value invalid).",missing_document_name:"Document name is missing.",missing_object:"Document data is missing.",missing_callback:"Missing callback function.",missing_blobid:"Blobid (key) is missing.",missing_title:"Document is missing a title.",missing_blob:"Document is missing a blob property.",invalid_json:"WARNING: Save object property {key} "+"with constructor: {ctor}.",doc_unsaved:"Document must be saved before "+"children can be saved."};function URL(url){this.url=url;this.params=[];}
...
return url+docid+'/'+blobid;},errorHandler:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.client.onError(code,message);},initChannel:function(fnSuccess){fnSuccess=fnSuccess||function(){};if(typeof goog=='undefined'||typeof goog.appengine=='undefined'){loader.loadScript('/_ah/channel/jsapi',this.initChannel.fnMethod(this).fnArgs(fnSuccess));return;}
if(this.client.username==undefined){this.client.onError('channel/user',"You must be signed in to receive notifications.");return;}
var self=this;this.client.onInfo('channel/init',"Intializing new channel.");$.ajax({url:'/channel/',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){result.expires=new Date().getTime()+
1000*result.lifetime;self.channelInfo=result;self.channel=new goog.appengine.Channel(result.token);self.socket=self.channel.open();self.socket.onmessage=self.onChannel.fnMethod(self);self.socket.onopen=function(){self.client.onInfo('channel/open',"Channel socket is open.");fnSuccess(self.channelInfo);};self.socket.onclose=function(){self.client.onError('channel/closed',"Realtime messages from PageForest "+"are no longer available.");delete self.channelInfo;delete self.channel;delete self.socket;};}});},onChannel:function(evt){var message=JSON.parse(evt.data);this.client.onInfo('channel/message',message.key+' ('+message.method+')');var sub=this.subscriptions[message.key];if(sub==undefined){this.client.onError('channel/nosub',"No subscription for channel key: "+
message.key);return;}
if(sub.enabled){sub.fn(message);}},subscribe:function(docid,blobid,options,fn){if(!this.validateArgs('subscribe',docid,blobid,undefined,options,fn)){return;}
options=options||{};options.enabled=(fn!=undefined);options.fn=fn;if(fn==undefined){options.enabled=false;}
var key=docid+'/';if(blobid!=undefined){key+=blobid+'/';}
if(options.exclusive){delete options.exclusive;this.subscriptions={};}
var sub=this.subscriptions[key]||{enabled:false};var subNew=util.extendObject({},sub,{enabled:true},options);this.subscriptions[key]=subNew;if(sub.enabled!=subNew.enabled){this.ensureSubs();}},hasSubscription:function(docid,blobid){var key=docid+'/';if(blobid!=undefined){key+=blobid+'/';}
...
var self=this;$.ajax({type:'PUT',url:'/channel/subscriptions',dataType:'json',data:jsonToString(this.subscriptions),error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){self.client.onInfo('channel/updated',"Subscriptions updated: "+
...
(blobid?'/'+blobid:''));return true;},putDoc:function(docid,json,fnSuccess){if(!this.validateArgs('putDoc',docid,undefined,json,undefined,fnSuccess)){return;}
...
this.appHost=window.location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.state='init';this.username=undefined;this.fLogging=true;this.logged={};this.lastHash='';this.fFirstPoll=true;this.saveInterval=60;this.autoLoad=false;setInterval(this.poll.fnMethod(this),ns.pollInterval);$(window).unload(this.beforeUnload.fnMethod(this));}
Client.methods({getDocURL:function(blobid){if(this.docid==undefined){return undefined;}
return this.storage.getDocURL(this.docid,blobid);},load:function(docid){if(this.app.setDoc==undefined){this.log(noSetDocMessage,{level:'warn',once:true});return;}
...
this.stateSave=this.state;this.changeState('saving');var self=this;this.storage.putDoc(docid,json,function(result){result.docid=docid;result.owner=json.owner||this.username;self.onSaveSuccess(result);});},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner','sha1']));this.setCleanDoc(result.docid);this.setAppPanelValues(this.meta);if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=typeof this.app.getDoc=='function'&&this.app.getDoc();if(typeof doc!='object'){this.log(noGetDocMessage,{level:'warn',once:true});doc={};}
...
return doc;},setDoc:function(doc){this.meta=base.project(doc,docProps);this.setAppPanelValues(this.meta);this.app.setDoc(doc);this.setCleanDoc(doc.doc_id);},onAutoLoad:function(message){if(!this.autoLoad||message.key!=this.docid+'/'||message.data.modified.isoformat==this.meta.modified.isoformat){this.log(autoLoadError+message.key);return;}
this.load(this.docid);},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=storage.jsonToString(this.getDoc());if(this.autoLoad&&this.docid!=undefined){if(!this.storage.hasSubscription(this.docid)){this.storage.subscribe(this.docid,undefined,{exclusive:true},this.onAutoLoad.fnMethod(this));}}
if(preserveHash){this.lastHash='';return;}
if(docid==undefined){docid='';}
window.location.hash=docid;this.lastHash=window.location.hash;},checkDoc:function(){if(this.saveInterval==0){return;}
...
this.log(message+' ('+code+')',{'obj':xmlhttp});this.onError(code,message);},onError:function(status,message){this.log(message+' ('+status+')');this.showError(message);if(this.app.onError){this.app.onError(status,message);}},onInfo:function(code,message){this.log(message+' ('+code+')');if(this.app.onInfo){this.app.onInfo(code,message);}},poll:function(){if(this.state=='init'){if(this.getDoc){this.setCleanDoc(undefined,true);}}
