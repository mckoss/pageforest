...
namespace.lookup('com.pageforest.storage').defineOnce(function(ns){var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var signInMessage="You must sign in to save a document.";var noClientMessage="App error: no client object available.";var docidMessage="Document name is missing.";var objectMessage="Document data is missing.";var titleMessage="Document is missing a title.";var blobMessage="Document is missing a blob property.";var invalidJSON="WARNING: Save object property {key} "+"with constructor: {ctor}.";var docUnsavedMessage="Document must be saved before "+"children can be saved.";function jsonToString(json){var s;var badProperty;function mapper(key,value){if(badProperty){return value;}
...
Storage.methods({getDocURL:function(docid,blobid){if(docid==undefined){return undefined;}
if(blobid==undefined){blobid='';}
return'http://'+this.client.appHost+'/docs/'+
docid+'/'+blobid;},errorHandler:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.client.log(message+' ('+code+')',{'obj':xmlhttp});this.client.onError(code,message);},putDoc:function(docid,json,fnSuccess){fnSuccess=fnSuccess||function(){};var validations={'no_username':[this.client.username,signInMessage],'missing_document_name':[docid,docidMessage],'missing_object':[json,objectMessage],'missing_title':[json&&json.title,titleMessage],'missing_blob':[json&&json.blob,blobMessage]};for(var code in validations){if(validations.hasOwnProperty(code)){var validation=validations[code];if(!validation[0]){this.client.onError(code,validation[1]);return;}}}
if(!json.readers){json.readers=['public'];}
var data=jsonToString(json);this.client.log('putDoc: '+docid);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getDoc:function(docid,fnSuccess){this.client.log("getDoc: "+docid);$.ajax({dataType:'json',url:this.getDocURL(docid),error:this.errorHandler.fnMethod(this),success:function(doc,textStatus,xmlhttp){fnSuccess(doc,textStatus,xmlhttp);}.fnMethod(this)});},putBlob:function(docid,blobid,data,options,fnSuccess){fnSuccess=fnSuccess||function(){};options=options||{};if(docid==undefined){this.client.onError('unsaved_document',docUnsavedMessage);return;}
...
this.client.log('saving blob: '+url+' ('+data.length+')');var self=this;$.ajax({type:'PUT',url:url,data:data,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(){self.client.log('saved blob: '+url);fnSuccess();}.fnMethod(this)});},getBlob:function(docid,blobid,options,fnSuccess){fnSuccess=fnSuccess||function(){};options=options||{};var type='GET';if(docid==undefined){this.client.onError('unsaved_document',docUnsavedMessage);fnSuccess(false);return;}
...
this.client.log('reading blob: '+docid+'/'+blobid);var self=this;$.ajax({'type':type,'url':url,'dataType':options.dataType||'json','error':function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;self.client.log(message+' ('+code+')',{'obj':xmlhttp});self.client.onError(code,message);fnSuccess(false);}.fnMethod(this),'success':function(data){self.client.log('read blob: '+url);fnSuccess(true,data);}.fnMethod(this)});}});ns.extend({'Storage':Storage,'jsonToString':jsonToString});});
...
this.app=app;this.storage=new storage.Storage(this);this.getDocURL=this.storage.getDocURL.fnMethod(this.storage);this.meta={};this.metaDoc={};this.metaDialog={};if(typeof $!='function'||$!=jQuery){this.onError('jQuery_required',jQueryMessage);return;}
this.appHost=window.location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.state='init';this.username=undefined;this.fLogging=true;this.logged={};this.fFirstPoll=true;this.saveInterval=60;setInterval(this.poll.fnMethod(this),ns.pollInterval);$(window).unload(this.beforeUnload.fnMethod(this));}
Client.methods({load:function(docid){if(this.app.setDoc==undefined){this.log(noSetDocMessage,{level:'warn'});return;}
...
this.stateSave=this.state;this.changeState('saving');var self=this;this.storage.putDoc(docid,json,function(result){result.docid=docid;result.owner=json.owner||this.username;self.onSaveSuccess(result);});},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner']));this.setCleanDoc(result.docid);this.setAppPanelValues(this.meta);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=typeof this.app.getDoc=='function'&&this.app.getDoc();if(typeof doc!='object'){this.log(noGetDocMessage,{level:'warn',once:true});doc={};}
...
this.checkUsername();this.checkDoc();this.fFirstPoll=false;},checkUsername:function(){var sessionUser=cookies.getCookie('sessionuser');if(sessionUser!=undefined){if(sessionUser!=this.username){this.username=sessionUser;this.log('signed in as '+this.username);this.onUserChange(this.username);}
return;}
...
var rcAppBox=dom.getRect($('#pfAppBarBox')[0]);dom.slide(this.appPanel,vector.lr(rcAppBox),animation,fnCallback);},showError:function(message){if(this.errorPanel==undefined){console.log("Error: "+message);return;}
