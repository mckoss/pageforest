...
this.log('saving blob: '+docid+'/'+blobid+' ('+data.length+')');$.ajax({type:'PUT',url:url,data:data,dataType:'json',processData:false,error:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',{'obj':xmlhttp});this.errorReport(code,message);fn(false);}.fnMethod(this),success:function(){this.log('saved blob: '+blobid);fn(true);}.fnMethod(this)});},getBlob:function(docid,blobid,options,fn){fn=fn||function(){};options=options||{};if(docid==undefined){this.errorReport('unsaved_document',docUnsavedMessage);fn(false);return;}
var url=this.getDocURL(docid)+blobid;if(options.encoding||options.tags){url+='?';}
if(options.encoding){url+='transfer-encoding='+options.encoding;}
if(options.tags){var encodedTags=base.map(options.tags,encodeURIComponent);url+='tags='+encodedTags.join(',');}
