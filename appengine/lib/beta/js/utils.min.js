...
namespace.lookup('com.pageforest.storage').defineOnce(function(ns){var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var client;var signInMessage="You must sign in to save a document.";var docidMessage="Document name is missing.";var objectMessage="Document data is missing.";var titleMessage="Document is missing a title.";var blobMessage="Document is missing a blob property.";var invalidJSON="WARNING: Save object property {key} "+"with constructor: {ctor}.";var docUnsavedMessage="Document must be saved before "+"children can be saved.";function setClient(clientT){client=clientT;}
function getDocURL(docid,blobid){if(docid==undefined){return undefined;}
if(blobid==undefined){blobid='';}
return'http://'+client.appHost+'/docs/'+docid+'/'+blobid;}
function jsonToString(json){var s;var badProperty;function mapper(key,value){if(badProperty){return value;}
...
return s;}
function errorHandler(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;client.log(message+' ('+code+')',{'obj':xmlhttp});client.onError(code,message);}
function putDoc(docid,json,fnSuccess){fnSuccess=fnSuccess||function(){};var validations={'no_username':[client.username,signInMessage],'missing_document_name':[docid,docidMessage],'missing_object':[json,objectMessage],'missing_title':[json&&json.title,titleMessage],'missing_blob':[json&&json.blob,blobMessage]};for(var code in validations){if(validations.hasOwnProperty(code)){var validation=validations[code];if(!validation[0]){client.onError(code,validation[1]);return;}}}
if(!json.readers){json.readers=['public'];}
var data=jsonToString(json);client.log('saving document: '+getDocURL(docid),json);$.ajax({type:'PUT',url:getDocURL(docid),data:data,error:errorHandler,success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});}
function putBlob(docid,blobid,data,options,fnSuccess){fnSuccess=fnSuccess||function(){};options=options||{};if(docid==undefined){client.onError('unsaved_document',docUnsavedMessage);return;}
var url=getDocURL(docid)+blobid;var query_string=[];function pushParam(param,value){query_string.push(param+'='+encodeURIComponent(value));}
...
client.log('saving blob: '+url+' ('+data.length+')');$.ajax({type:'PUT',url:url,data:data,dataType:'json',processData:false,error:errorHandler,success:function(){client.log('saved blob: '+url);fnSuccess(true);}.fnMethod(this)});}
function getBlob(docid,blobid,options,fn){fn=fn||function(){};options=options||{};var type='GET';if(docid==undefined){client.onError('unsaved_document',docUnsavedMessage);fn(false);return;}
var url=getDocURL(docid)+blobid;if(options.encoding||options.tags){url+='?';}
...
client.log('reading blob: '+docid+'/'+blobid);$.ajax({'type':type,'url':url,'dataType':options.dataType||'json','error':function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;client.log(message+' ('+code+')',{'obj':xmlhttp});client.onError(code,message);fn(false);}.fnMethod(this),'success':function(data){client.log('read blob: '+url);fn(true,data);}.fnMethod(this)});}
ns.extend({'setClient':setClient,'putDoc':putDoc,'getDoc':function(){console.log("NYI");},'putBlob':putBlob,'getBlob':getBlob,'getDocURL':getDocURL,'jsonToString':jsonToString});});
/* Begin file: client.js */
namespace.lookup('com.pageforest.client').defineOnce(function(ns){var storage=namespace.lookup('com.pageforest.storage');var cookies=namespace.lookup('org.startpad.cookies');var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var dom=namespace.lookup('org.startpad.dom');var dialog=namespace.lookup('org.startpad.dialog');var vector=namespace.lookup('org.startpad.vector');ns.pollInterval=1000;var discardMessage="You will lose your document changes if you continue.";var jQueryMessage="jQuery must be installed to use this library.";var unloadMessage="You will lose your changes if you leave "+"the document without saving.";var noSetDoc="This app does not have a setDoc method "+"and so cannot be loaded.";var docProps=['title','tags','owner','readers','writers','created','modified'];function Client(app){storage.setClient(this);this.app=app;this.meta={};this.metaDoc={};this.metaDialog={};if(typeof $!='function'||$!=jQuery){this.onError('jQuery_required',jQueryMessage);return;}
...
setInterval(this.poll.fnMethod(this),ns.pollInterval);$(window).unload(this.beforeUnload.fnMethod(this));}
Client.methods({load:function(docid){if(this.app.setDoc==undefined){this.log(noSetDoc);return;}
if(this.isDirty()){if(!this.confirmDiscard()){return;}
this.changeState('clean');}
this.stateSave=this.state;this.changeState('loading');this.log("loading: "+docid);$.ajax({dataType:'json',url:storage.getDocURL(docid),error:this.errorHandler.fnMethod(this),success:function(doc,textStatus,xmlhttp){this.setDoc(doc);this.setCleanDoc(docid);}.fnMethod(this)});},save:function(json,docid){if(this.isSaved()){return;}
...
this.stateSave=this.state;this.changeState('saving');var self=this;storage.putDoc(docid,json,function(result){result.docid=docid;result.owner=json.owner||this.username;self.onSaveSuccess(result);});},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner']));this.setCleanDoc(result.docid);this.setAppPanelValues(this.meta);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=this.app.getDoc();if(typeof doc!='object'){doc={};}
...
return;}
if(this.username||this.fFirstPoll){this.username=undefined;this.onUserChange(this.username);}},onUserChange:function(username){if(this.app.onUserChange){this.app.onUserChange(username);}
