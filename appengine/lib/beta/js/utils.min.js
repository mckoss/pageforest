...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},putBlob:function(blobid,data,encoding,docid){if(this.docid==undefined){this.errorReport('unsaved_document',docUnsavedMessage);return;}
var url=this.getDocURL()+blobid;if(encoding){url+='?transfer-encoding='+encoding;}
this.log('saving blob: '+blobid+' ('+data.length+')');$.ajax({type:'PUT',url:url,data:data,dataType:'json',processData:false,error:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',{'obj':xmlhttp});this.errorReport(code,message);}.fnMethod(this),success:function(data){this.log('saved blob: '+blobid);if(this.app.onBlobSaveSuccess){this.app.onBlobSaveSuccess();}}.fnMethod(this)});},checkExists:function(docid,blobid,fn){var url=this.getDocURL(docid,blobid);},detach:function(){this.setCleanDoc();this.setDirty();},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=JSON.stringify(this.app.getDoc());if(preserveHash){this.lastHash='';return;}
if(docid==undefined){docid='';}
location.hash=docid;this.lastHash=location.hash;location.href=location.href;},getDocURL:function(docid,blobid){if(docid==undefined){docid=this.docid;}
...
else{var formatted="client error: "+message+' ('+status+')';console.log(formatted);}},confirmDiscard:function(){if(this.app.confirmDiscard){return this.app.confirmDiscard();}
...
var json=JSON.stringify(this.app.getDoc());if(json!=this.lastJSON){this.setDirty();}},signIn:function(){window.open('http://'+this.wwwHost+'/sign-in/'+
