...
options=options||{};options.enabled=(fn!=undefined);options.fn=fn;var key=docid+'/';if(blobid!=undefined){key+=blobid+'/';}
if(options.exclusive){delete options.exclusive;this.subscriptions={};}
var sub=this.subscriptions[key]||{enabled:false};var subNew=util.extendObject({},sub,{enabled:true},options);this.subscriptions[key]=subNew;this.ensureSubs();},hasSubscription:function(docid,blobid){var key=docid+'/';if(blobid!=undefined){key+=blobid+'/';}
