...
var reToken=/\{([^}]+)\}/g;function format(st){st=st.toString();var args=Array.prototype.slice.call(arguments,1);if(args.length==1&&typeof args[0]=='object'){args=args[0];}
st=st.replace(reToken,function(whole,key){var value=args;var keys=key.split('.');for(var i=0;i<keys.length;i++){key=keys[i];var n=parseInt(key);if(!isNaN(n)){value=value[n];}else{value=value[key];}
if(value==undefined){console.log("format error: "+keys.slice(0,i+1).join('.'));return"";}}
return value;});return st;}
String.prototype.format=function(){var args=util.copyArray(arguments);args.splice(0,0,this);return format.apply(undefined,args);};ns.extend({'fixedDigits':fixedDigits,'thousands':thousands,'slugify':slugify,'escapeHTML':escapeHTML,'format':format,'replaceKeys':format,'replaceString':replaceString,'base64ToString':base64ToString,'canvasToPNG':canvasToPNG,'dateFromISO':dateFromISO,'isoFromDate':isoFromDate,'setTimezone':setTimezone,'decodeClass':decodeClass,'shortDate':shortDate,'wordList':wordList,'arrayFromWordList':arrayFromWordList,'repeat':repeat});});
