...
if(options.encoding){url+='transfer-encoding='+options.encoding;}
if(options.tags){var encodedTags=base.map(options.tags,encodeURIComponent);url+='tags='+encodedTags.join(',');}
this.log('reading blob: '+docid+'/'+blobid);$.ajax({type:'GET',url:url,dataType:options.dataType||'json',error:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',{'obj':xmlhttp});this.errorReport(code,message);fn(false);}.fnMethod(this),success:function(data){this.log('saved blob: '+blobid);fn(true,data);}.fnMethod(this)});},getDocURL:function(docid,blobid){if(docid==undefined){docid=this.docid;}
