...
function distance2(v1,v2){var d2=0;for(var i=0;i<v1.length;i++){d2+=Math.pow((v2[i]-v1[i]),2);}
return d2;}
...
ns.extend({'x':x,'y':y,'x2':x2,'y2':y2,'equal':equal,'sub':sub,'subFrom':subFrom,'add':add,'addTo':addTo,'max':max,'mult':mult,'floor':floor,'dotProduct':dotProduct,'ul':ul,'lr':lr,'copy':copy,'append':copy,'size':size,'numInRange':numInRange,'clipToRange':clipToRange,'ptInRect':ptInRect,'ptClipToRect':ptClipToRect,'rcClipToRect':rcClipToRect,'ptCenter':ptCenter,'boundingBox':boundingBox,'ptRegistration':ptRegistration,'rcExpand':rcExpand,'alignRect':alignRect,'keepInRect':keepInRect,'iRegClosest':iRegClosest,'rcDeltaReg':rcDeltaReg});});
/* Begin file: dom.js */
...
return s;}
function Storage(client){this.client=client;this.subscriptions={};}
...
return url+docid+'/'+blobid;},errorHandler:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.client.log(message+' ('+code+')',{'obj':xmlhttp});this.client.onError(code,message);},initChannel:function(fnSuccess){fnSuccess=fnSuccess||function(){};var self=this;console.log("Intializing new channel");$.ajax({url:'/channel/',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){result.expires=new Date().getTime()+
1000*result.lifetime;self.channel=new goog.appengine.Channel(result.token);self.socket=self.channel.open();self.socket.onmessage=self.onChannel.fnMethod(self);self.channelInfo=result;fnSuccess(result);}});},onChannel:function(evt){var message=JSON.parse(evt.data);console.log("onChannel: ",message);var sub=this.subscriptions[message.key];if(sub==undefined){console.log("No subscription for channel key.",message.key);return;}
sub.fn(message);},subscribe:function(docid,blobid,options,fn){if(!this.validateArgs('subscribe',docid,blobid,undefined,options,fn)){return;}
var sub;var key=docid+'/';if(blobid!=undefined){key+=blobid+'/';}
if(fn==undefined||options&&options.enabled===false){sub=this.subscriptions[key];if(sub==undefined){return;}
sub.enabled=false;this.ensureSubs();return;}
sub=this.subscriptions[key]||{enabled:true};util.extendObject(sub,options);sub.fn=fn;this.subscriptions[key]=sub;this.ensureSubs();},ensureSubs:function(){if(this.channelInfo==undefined||this.channelInfo.expires<new Date().getTime()){this.initChannel(this.ensureSubs.fnMethod(this));return;}
var self=this;$.ajax({type:'PUT',url:'/channel/subscriptions',dataType:'json',data:jsonToString(this.subscriptions),error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){console.log("Subscriptions updated.");}});},validateArgs:function(funcName,docid,blobid,json,options,fnSuccess){var blobFuncs=['getBlob','putBlob','push','slice'];var isPutMethod=funcName.indexOf('put')==0||funcName=='push';var isBlobMethod=base.indexOf(funcName,blobFuncs)!=-1;var validations={no_username:!isPutMethod||this.client.username!=undefined,missing_object:!isPutMethod||json!=undefined,bad_options:typeof options!='function',bad_callback:fnSuccess==undefined||typeof fnSuccess=='function',slice_range:options==undefined||(options.start==undefined||typeof options.start=='number')&&(options.end==undefined||typeof options.end=='number'),missing_document_name:funcName=='list'||docid!=undefined,missing_callback:isPutMethod||fnSuccess!=undefined,missing_blobid:!isBlobMethod||blobid!=undefined,missing_title:funcName!='putDoc'||typeof json=='object'&&json.title,missing_blob:funcName!='putDoc'||typeof json=='object'&&json.blob};for(var code in validations){if(validations.hasOwnProperty(code)){var validation=validations[code];if(!validation){this.client.onError(code,errorMessages[code]+'('+funcName+')');return false;}}}
var obj={docid:docid,blobid:blobid};if(json){obj.jsonLength=jsonToString(json).length;}
util.extendObject(obj,options);this.client.log(funcName+': '+
JSON.stringify(obj));return true;},putDoc:function(docid,json,fnSuccess){if(!this.validateArgs('putDoc',docid,undefined,json,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};if(!json.readers){json.readers=['public'];}
var data=jsonToString(json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getDoc:function(docid,fnSuccess){if(!this.validateArgs('getDoc',docid,undefined,undefined,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};$.ajax({dataType:'json',url:this.getDocURL(docid),error:this.errorHandler.fnMethod(this),success:function(doc,textStatus,xmlhttp){fnSuccess(doc,textStatus,xmlhttp);}});},deleteDoc:function(docid,fnSuccess){if(!this.validateArgs('deleteDoc',docid,undefined,undefined,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};$.ajax({type:'PUT',dataType:'json',url:this.getDocURL(docid)+'?method=delete',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},putBlob:function(docid,blobid,data,options,fnSuccess){if(!this.validateArgs('putBlob',docid,blobid,data,options,fnSuccess)){return;}
...
$.ajax({type:'PUT',url:url.toString(),data:data,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},push:function(docid,blobid,json,options,fnSuccess){if(!this.validateArgs('push',docid,blobid,json,options,fnSuccess)){return;}
...
$.ajax({type:'PUT',url:url.toString(),data:json,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getBlob:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('getBlob',docid,blobid,undefined,options,fnSuccess)){return;}
...
$.ajax({type:type,url:url.toString(),dataType:options.dataType||'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},slice:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('slice',docid,blobid,undefined,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};var url=new URL(this.getDocURL(docid,blobid));url.push('method','slice');url.push('start',options.start);url.push('end',options.end);url.push('wait',options.wait);url.push('etag',options.etag);$.ajax({url:url.toString(),dataType:'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},deleteBlob:function(docid,blobid,fnSuccess){if(!this.validateArgs('deleteBlob',docid,blobid,undefined,undefined,fnSuccess)){return;}
...
$.ajax({url:url.toString(),dataType:'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});}});ns.extend({'Storage':Storage,'jsonToString':jsonToString,'getEtag':getEtag});});
...
this.stateSave=this.state;this.changeState('loading');var self=this;this.storage.getDoc(docid,function(doc){self.setDoc(doc);});},save:function(json,docid){if(this.isSaved()){return;}
...
return doc;},setDoc:function(doc){this.meta=base.project(doc,docProps);this.setAppPanelValues(this.meta);this.app.setDoc(doc);this.setCleanDoc(doc.doc_id);},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=storage.jsonToString(this.getDoc());if(preserveHash){this.lastHash='';return;}
