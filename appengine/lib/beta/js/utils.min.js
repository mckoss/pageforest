...
var data=jsonToString(json);this.client.log('putDoc: '+docid+' ('+data.length+' bytes)');$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getDoc:function(docid,fnSuccess){if(!this.validateArgs('getDoc',docid,undefined,undefined,undefined,fnSuccess)){return;}
...
var url=new Query(this.getDocURL(docid,blobid));if(options.encoding){url.push('transfer-encoding',options.encoding);}
if(options.tags){url.push('tags',options.tags.join(','));}
if(typeof data!="string"){data=jsonToString(data);}
this.client.log('putBlob: '+docid+'/'+blobid+'?'+url.params.join(', ')+' ('+data.length+' bytes)');$.ajax({type:'PUT',url:url.toString(),data:data,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},push:function(docid,blobid,json,options,fnSuccess){if(!this.validateArgs('push',docid,blobid,json,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};var url=new Query(this.getDocURL(docid,blobid));url.push('method','push');url.push('max',options.max);if(docid==undefined){this.client.onError('unsaved_document',docUnsavedMessage);return;}
if(typeof json!="string"){json=jsonToString(json);}
this.client.log('push: '+docid+'/'+blobid+' ('+json.length+' bytes)');$.ajax({type:'PUT',url:url.toString(),data:json,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getBlob:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('getBlob',docid,blobid,undefined,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};options=options||{};var url=new Query(this.getDocURL(docid,blobid));url.push('transfer-encoding',options.encoding);var type='GET';if(options.headOnly){type='HEAD';}
this.client.log('getBlob: '+docid+'/'+blobid);$.ajax({type:type,url:url.toString(),dataType:options.dataType||'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},slice:function(docid,blobid,start,end,fnSuccess){if(!this.validateArgs('slice',docid,blobid,undefined,{start:start,end:end},fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};var url=new Query(this.getDocURL(docid,blobid));url.push('method','slice');url.push('start',start);url.push('end',end);this.client.log('slice: '+docid+'/'+blobid+'['+(start||'')+':'+
(end||'')+']');$.ajax({url:url.toString(),dataType:'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},deleteBlob:function(docid,blobid,fnSuccess){if(!this.validateArgs('deleteBlob',docid,blobid,undefined,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};this.client.log('deleteBlob: '+docid+'/'+blobid);$.ajax({type:'PUT',dataType:'json',url:this.getDocURL(docid,blobid)+'?method=delete',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},list:function(docid,blobid,options,fnSuccess){var simpleOptions=['depth','keysonly','prefix','tag'];if(!this.validateArgs('list',docid,undefined,undefined,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};options=options||{};var url=new Query(this.getDocURL(docid,blobid));url.push('method','list');for(var i=0;i<simpleOptions.length;i++){var option=simpleOptions[i];if(options[option]!=undefined){url.push(option,options[option]);}}
url.push('transfer-encoding',options.encoding);if(options.tags){url.push('tag',options.tags.join(','));}
this.client.log('list: '+docid+'/ ('+url.params.join(', ')+')');$.ajax({url:url.toString(),dataType:'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});}});ns.extend({'Storage':Storage,'jsonToString':jsonToString});});
