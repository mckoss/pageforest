...
return url+docid+'/'+blobid;},errorHandler:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.client.onError(code,message);},initChannel:function(fnSuccess){fnSuccess=fnSuccess||function(){};var self=this;this.client.onInfo('channel/init',"Intializing new channel.");$.ajax({url:'/channel/',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){result.expires=new Date().getTime()+
...
this.log(message+' ('+code+')',{'obj':xmlhttp});this.onError(code,message);},onError:function(status,message){this.log("client error: "+message+' ('+status+')');this.showError(message);if(this.app.onError){this.app.onError(status,message);}},onInfo:function(code,message){this.log(code+' ('+message+')');if(this.app.onInfo){this.app.onInfo(code,message);}},poll:function(){if(this.state=='init'){if(this.getDoc){this.setCleanDoc(undefined,true);}}
