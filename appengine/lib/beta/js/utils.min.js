...
this.setValues(initialValues);},getField:function(name){this.bindFields();for(var i=0;i<this.fields.length;i++){if(this.fields[i].name==name){return this.fields[i];}}
return undefined;},hasChanged:function(name){var values=this.getValues();return values[name]!=this.lastValues[name];},setValues:function(values){var field;base.extendObject(this.lastValues,values);for(var name in values){if(values.hasOwnProperty(name)){field=this.getField(name);if(field==undefined||field.elt==undefined){continue;}
...
var field=this.getField(name);switch(field.elt.tagName){case'INPUT':case'TEXTAREA':field.elt.disabled=!enabled;break;case'DIV':field.elt.style.display=enabled?'block':'none';break;default:throw new Error("Field "+name+" is not a form field.");}}});ns.extend({'Dialog':Dialog,'styles':styles});});
