...
Function.prototype.methods=function(obj){extendObject(this.prototype,obj);};Function.methods({fnMethod:function(obj){var _fn=this;return function(){return _fn.apply(obj,arguments);};},fnArgs:function(){var _fn=this;var _args=copyArray(arguments);return function(){var args=copyArray(arguments).concat(_args);var self=this;return _fn.apply(self,args);};},fnWrap:function(fn){var _fn=this;return function(){var self=this;return _fn(self,fn,arguments);};}});Namespace.methods({define:function(callback){this._isDefined=true;if(namespaceT.logLevel<=1){console.info("Namespace '"+this._path+"' defined.");}
if(callback){Namespace.defining=this;callback(this);Namespace.defining=undefined;}
...
return a;}
function valueInArray(value,a){a=ensureArray(a);for(var i=0;i<a.length;i++){if(value==a[i]){return true;}}
return false;}
...
ns.extend({'extendObject':util.extendObject,'Enum':Enum,'StBuf':StBuf,'extendIfMissing':extendIfMissing,'extendIfChanged':extendIfChanged,'extendDeep':extendDeep,'randomInt':randomInt,'strip':strip,'project':project,'uniqueArray':uniqueArray,'valueInArray':valueInArray,'map':map,'filter':filter,'reduce':reduce,'keys':keys,'forEach':forEach,'ensureArray':ensureArray,'isEqual':isEqual});});
...
var tzDefault=-(new Date().getTimezoneOffset())/60;function isoFromDate(dt,fTime){var dtT=new Date();dtT.setTime(dt.getTime());var tz=dt.__tz;if(tz==undefined){tz=tzDefault;}
...
ns.extend({'fixedDigits':fixedDigits,'thousands':thousands,'slugify':slugify,'escapeHTML':escapeHTML,'replaceKeys':replaceKeys,'replaceString':replaceString,'base64ToString':base64ToString,'canvasToPNG':canvasToPNG,'dateFromISO':dateFromISO,'isoFromDate':isoFromDate,'decodeClass':decodeClass,'shortDate':shortDate,'wordList':wordList,'arrayFromWordList':arrayFromWordList});});
/* Begin file: vector.js */
namespace.lookup('org.startpad.vector').defineOnce(function(ns){var util=namespace.util;ns.extend({x:0,y:1,x2:2,y2:3,regNums:{'ul':0,'top':1,'ur':2,'left':3,'center':4,'right':5,'ll':6,'bottom':7,'lr':8},subFrom:function(v1,v2){for(var i=0;i<v1.length;i++){v1[i]=v1[i]-v2[i%v2.length];}
return v1;},sub:function(v1,v2){var vDiff=ns.copy(v1);return ns.subFrom(vDiff,v2);},addTo:function(vSum){for(var iarg=1;iarg<arguments.length;iarg++){var v=arguments[iarg];for(var i=0;i<vSum.length;i++){vSum[i]+=v[i%v.length];}}
return vSum;},add:function(){var vSum=ns.copy(arguments[0]);var args=util.copyArray(arguments);args[0]=vSum;return ns.addTo.apply(undefined,args);},max:function(){var vMax=ns.copy(arguments[0]);for(var iarg=1;iarg<arguments.length;iarg++){var v=arguments[iarg];for(var i=0;i<vMax.length;i++){if(v[i]>vMax[i]){vMax[i]=v[i];}}}
return vMax;},mult:function(){var vProd=1;var i;for(var iarg=0;iarg<arguments.length;iarg++){var v=arguments[iarg];if(typeof v==="number"){if(typeof vProd==="number"){vProd*=v;}
else{for(i=0;i<vProd.length;i++){vProd[i]*=v;}}}
else{if(typeof vProd==="number"){var vT=vProd;vProd=ns.copy(v);for(i=0;i<vProd.length;i++){vProd[i]*=vT;}}
else{if(v.length!==vProd.length){throw new Error("Mismatched Vector Size");}
for(i=0;i<vProd.length;i++){vProd[i]*=v[i];}}}}
return vProd;},floor:function(v){var vFloor=[];for(var i=0;i<v.length;i++){vFloor[i]=Math.floor(v[i]);}
return vFloor;},dotProduct:function(){var v=ns.mult.apply(undefined,arguments);var s=0;for(var i=0;i<v.length;i++){s+=v[i];}
return s;},append:function(){var v1=Array.prototype.concat.apply([],arguments);return v1;},equal:function(v1,v2){if(v1.length!=v2.length){return false;}
for(var i=0;i<v1.length;i++){if(typeof v1[i]!=typeof v2[i]){return false;}
if(typeof v1[i]=="object"){if(!ns.equal(v1[i],v2[i])){return false;}}else{if(v1[i]!=v2[i]){return false;}}}
return true;},ul:function(rc){return rc.slice(0,2);},lr:function(rc){return rc.slice(2,4);},size:function(rc){return ns.sub(ns.lr(rc),ns.ul(rc));},numInRange:function(num,numMin,numMax){return num>=numMin&&num<=numMax;},clipToRange:function(num,numMin,numMax){if(num<numMin){return numMin;}
if(num>numMax){return numMax;}
return num;},ptInRect:function(pt,rc){return ns.numInRange(pt[ns.x],rc[ns.x],rc[ns.x2])&&ns.numInRange(pt[ns.y],rc[ns.y],rc[ns.y2]);},ptClipToRect:function(pt,rc){return[ns.clipToRange(pt[ns.x],rc[ns.x],rc[ns.x2]),ns.clipToRange(pt[ns.y],rc[ns.y],rc[ns.y2])];},rcClipToRect:function(rc,rcClip){return ns.append(ns.ptClipToRect(ns.ul(rc),rcClip),ns.ptClipToRect(ns.lr(rc),rcClip));},rcExpand:function(rc,ptSize){return ns.append(ns.sub(ns.ul(rc),ptSize),ns.add(ns.lr(rc),ptSize));},keepInRect:function(rcIn,rcBound){var ptFixSize=ns.max([0,0],ns.sub(ns.size(rcIn),ns.size(rcBound)));rcIn[ns.x2]-=ptFixSize[ns.x];rcIn[ns.y2]-=ptFixSize[ns.y];var dx=0;var dy=0;dx=Math.max(0,rcBound[ns.x]-rcIn[ns.x]);dy=Math.max(0,rcBound[ns.y]-rcIn[ns.y]);if(dx==0){dx=Math.min(0,rcBound[ns.x2]-rcIn[ns.x2]);}
if(dy==0){dy=Math.min(0,rcBound[ns.y2]-rcIn[ns.y2]);}
ns.addTo(rcIn,[dx,dy]);},ptCenter:function(rc,scale){if(scale===undefined){scale=0.5;}
if(typeof scale==="number"){scale=[scale,scale];}
var pt=ns.mult(scale,ns.lr(rc));scale=ns.sub([1,1],scale);ns.addTo(pt,ns.mult(scale,ns.ul(rc)));return pt;},ptRegistration:function(rc,reg){if(typeof reg=='string'){reg=ns.regNums[reg];}
var xScale=(reg%3)*0.5;var yScale=Math.floor(reg/3)*0.5;return ns.ptCenter(rc,[xScale,yScale]);},iRegClosest:function(pt,rc){var aPoints=[];for(var i=0;i<9;i++){aPoints.push(ns.ptRegistration(rc,i));}
return ns.IPtClosest(pt,aPoints)[0];},alignRect:function(rc,reg,ptTo){var ptFrom=ns.ptRegistration(rc,reg);return ns.add(rc,ns.sub(ptTo,ptFrom));},rectDeltaReg:function(rc,dpt,iReg,ptSizeMin,rcBounds){var rcT;if(iReg==4){rcT=ns.add(rc,dpt);if(rcBounds){ns.keepInRect(rcT,rcBounds);}
return rcT;}
...
var ptSize=ns.size(rcT);var ptFixSize=ns.max([0,0],ns.sub(ptSizeMin,ptSize));if(iX==0){ptFixSize[0]*=-1;}
if(iY==0){ptFixSize[1]*=-1;}
rcT=applyDelta(rcT,ptFixSize);if(rcBounds){ns.keepInRect(rcT,rcBounds);}
return rcT;},iPtClosest:function(pt){var d2Min;var ptClosest;var iClosest;var d2;var iPt=0;for(var iarg=1;iarg<arguments.length;iarg++){var v=arguments[iarg];if(typeof v[0]=="number"){d2=ns.Distance2(pt,v);if(d2Min==undefined||d2<d2Min){d2Min=d2;ptClosest=v;iClosest=iPt;}
iPt++;}
else{for(var i=0;i<v.length;i++){var vT=v[i];d2=ns.Distance2(pt,vT);if(d2Min==undefined||d2<d2Min){d2Min=d2;ptClosest=vT;iClosest=iPt;}
iPt++;}}}
return[iClosest,ptClosest];},distance2:function(v1,v2){var d2=0;for(var i=0;i<v1.length;i++){d2+=Math.pow((v2[i]-v1[i]),2);}
return d2;},boundingBox:function(){var vPoints=ns.append.apply(undefined,arguments);if(vPoints.length%2!==0){throw new Error("Invalid arguments to boundingBox");}
...
return[ptMin[0],ptMin[1],ptMax[0],ptMax[1]];},json:function(v){var sRect="[";var chSep="";for(var i=0;i<v.length;i++){sRect+=chSep+v[i];chSep=",";}
sRect+="]";return sRect;}});ns.copy=ns.append;});
/* Begin file: dom.js */
namespace.lookup('org.startpad.dom').define(function(ns){var util=namespace.util;var vector=namespace.lookup('org.startpad.vector');var base=namespace.lookup('org.startpad.base');var ix=0;var iy=1;var ix2=2;var iy2=3;function getPos(elt){var pt=[0,0];while(elt.offsetParent!==null){pt[ix]+=elt.offsetLeft;pt[iy]+=elt.offsetTop;elt=elt.offsetParent;}
return pt;}
...
if(field.onClick!=undefined){dom.bind(field.elt,'click',function(){field.onClick();});}
...
return undefined;},setValues:function(values){var field;this.bindFields();for(var name in values){if(values.hasOwnProperty(name)){field=this.getField(name);if(field==undefined||field.elt==undefined){continue;}
...
namespace.lookup('com.pageforest.storage').defineOnce(function(ns){var base=namespace.lookup('org.startpad.base');var util=namespace.util;var format=namespace.lookup('org.startpad.format');var signInMessage="You must sign in to save a document.";var noClientMessage="App error: no client object available.";var docidMessage="Document name is missing.";var blobidMessage="Blobid (key) is missing.";var objectMessage="Document data is missing.";var callbackMessage="Missing callback function.";var sliceMessage="Invalid slice range (start or end value invalid).";var titleMessage="Document is missing a title.";var blobMessage="Document is missing a blob property.";var invalidJSON="WARNING: Save object property {key} "+"with constructor: {ctor}.";var docUnsavedMessage="Document must be saved before "+"children can be saved.";var badAPIMessage="API Call invalid";function jsonToString(json){var s;var badProperty;function mapper(key,value){if(badProperty){return value;}
if(typeof value=='object'&&value.constructor!=Object&&value.constructor!=Array){console.warn(format.replaceKeys(invalidJSON,{key:key,ctor:value.constructor.toString()}));badProperty=key;}
return value;}
try{s=JSON.stringify(json,mapper);}catch(e){console.error(e.message);return JSON.stringify({error:e.message});}
return s;}
function Storage(client){this.client=client;}
Storage.methods({getDocURL:function(docid,blobid){docid=docid||'';blobid=blobid||'';var url='http://'+this.client.appHost+'/docs/';if(docid==''){return url;}
return url+docid+'/'+blobid;},errorHandler:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.client.log(message+' ('+code+')',{'obj':xmlhttp});this.client.onError(code,message);},validateArgs:function(funcName,docid,blobid,json,options,fnSuccess){var validations={'no_username':[this.client.username!=undefined,signInMessage],'bad_options':[typeof options!='function',badAPIMessage],'bad_callback':[fnSuccess==undefined||typeof fnSuccess=='function',badAPIMessage]};if(funcName!='list'){util.extendObject(validations,{'missing_document_name':[docid!=undefined,docidMessage]});}
if(funcName=='putDoc'){util.extendObject(validations,{'missing_title':[typeof json=='object'&&json.title,titleMessage],'missing_blob':[typeof json=='object'&&json.blob,blobMessage]});}
if(funcName.indexOf('put')==0){validations['missing_object']=[json!=undefined,objectMessage];}
if(funcName.indexOf('get')==0||funcName=='list'||funcName=='slice'){validations['missing_callback']=[fnSuccess!=undefined,callbackMessage];}
if(funcName=='slice'){validations['slice_range']=[(options.start==undefined||typeof options.start=='number')&&(options.end==undefined||typeof options.end=='number'),sliceMessage];}
if(funcName.indexOf('Blob')!=-1){util.extendObject(validations,{'missing_blobid':[blobid!=undefined,blobidMessage]});}
for(var code in validations){if(validations.hasOwnProperty(code)){var validation=validations[code];if(!validation[0]){this.client.onError(code,funcName+': '+
validation[1]);return false;}}}
return true;},putDoc:function(docid,json,fnSuccess){if(!this.validateArgs('putDoc',docid,undefined,json,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};if(!json.readers){json.readers=['public'];}
var data=jsonToString(json);this.client.log('putDoc: '+docid+' ('+data.length+' bytes)');$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getDoc:function(docid,fnSuccess){this.client.log("getDoc: "+docid);$.ajax({dataType:'json',url:this.getDocURL(docid),error:this.errorHandler.fnMethod(this),success:function(doc,textStatus,xmlhttp){fnSuccess(doc,textStatus,xmlhttp);}});},deleteDoc:function(docid,fnSuccess){if(!this.validateArgs('deleteDoc',docid,undefined,undefined,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};this.client.log("deleteDoc: "+docid);$.ajax({type:'PUT',dataType:'json',url:this.getDocURL(docid)+'?method=delete',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},putBlob:function(docid,blobid,data,options,fnSuccess){if(!this.validateArgs('putBlob',docid,blobid,data,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};options=options||{};if(docid==undefined){this.client.onError('unsaved_document',docUnsavedMessage);return;}
var url=this.getDocURL(docid,blobid);var query_string=[];function pushParam(param,value){query_string.push(param+'='+encodeURIComponent(value));}
if(options.encoding){pushParam('transfer-encoding',options.encoding);}
if(options.tags){pushParam('tags',options.tags.join(','));}
query_string=query_string.join('&');if(query_string){url+='?'+query_string;}
if(typeof data!="string"){data=jsonToString(data);}
this.client.log('putBlob: '+docid+'/'+blobid+
(query_string?'?'+query_string:'')+' ('+data.length+' bytes)');$.ajax({type:'PUT',url:url,data:data,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},push:function(docid,blobid,json,fnSuccess){if(!this.validateArgs('putBlob',docid,blobid,json,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};if(docid==undefined){this.client.onError('unsaved_document',docUnsavedMessage);return;}
if(typeof json!="string"){json=jsonToString(json);}
this.client.log('push: '+docid+'/'+blobid+' ('+json.length+' bytes)');$.ajax({type:'PUT',url:this.getDocURL(docid,blobid)+'?method=push',data:json,dataType:'json',processData:false,error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},getBlob:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('getBlob',docid,blobid,undefined,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};options=options||{};var type='GET';var url=this.getDocURL(docid,blobid);if(options.encoding){url+='?transfer-encoding='+options.encoding;}
if(options.headOnly){type='HEAD';}
this.client.log('getBlob: '+docid+'/'+blobid);$.ajax({type:type,url:url,dataType:options.dataType||'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},slice:function(docid,blobid,start,end,fnSuccess){if(!this.validateArgs('slice',docid,blobid,undefined,{start:start,end:end},fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};var url=this.getDocURL(docid,blobid);var query_string=[];function pushParam(param,value){query_string.push(param+'='+encodeURIComponent(value));}
pushParam('method','slice');if(start!=undefined){pushParam('start',start);}
if(end!=undefined){pushParam('end',end);}
query_string=query_string.join('&');if(query_string){url+='?'+query_string;}
this.client.log('slice: '+docid+'/'+blobid+'['+(start||'')+':'+
(end||'')+']');$.ajax({url:url,dataType:'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},deleteBlob:function(docid,blobid,fnSuccess){if(!this.validateArgs('deleteBlob',docid,blobid,undefined,undefined,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};this.client.log('deleteBlob: '+docid+'/'+blobid);$.ajax({type:'PUT',dataType:'json',url:this.getDocURL(docid,blobid)+'?method=delete',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});},list:function(docid,blobid,options,fnSuccess){if(!this.validateArgs('list',docid,undefined,undefined,options,fnSuccess)){return;}
fnSuccess=fnSuccess||function(){};options=options||{};var url=this.getDocURL(docid,blobid);var query_string=[];function pushParam(param,value){query_string.push(param+'='+encodeURIComponent(value));}
pushParam('method','list');var simpleOptions=['depth','keysonly','prefix','tag'];for(var i=0;i<simpleOptions.length;i++){var option=simpleOptions[i];if(options[option]!=undefined){pushParam(option,options[option]);}}
if(options.encoding){pushParam('transfer-encoding',options.encoding);}
if(options.tags){pushParam('tag',options.tags.join(','));}
url+='?'+query_string.join('&');this.client.log('list: '+docid+'/ ('+query_string.join(', ')+')');$.ajax({url:url,dataType:'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});}});ns.extend({'Storage':Storage,'jsonToString':jsonToString});});
/* Begin file: client.js */
namespace.lookup('com.pageforest.client').defineOnce(function(ns){var storage=namespace.lookup('com.pageforest.storage');var cookies=namespace.lookup('org.startpad.cookies');var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var dom=namespace.lookup('org.startpad.dom');var dialog=namespace.lookup('org.startpad.dialog');var vector=namespace.lookup('org.startpad.vector');ns.pollInterval=1000;var discardMessage="You will lose your document changes if you continue.";var jQueryMessage="jQuery must be installed to use this library.";var unloadMessage="You will lose your changes if you leave "+"the document without saving.";var noSetDocMessage="This app does not have a setDoc method "+"and so cannot be loaded.";var noGetDocMessage="This app does not have a getDoc method "+"and so cannot be saved.";var noAppMessage="Warning: no app object provided - "+"only direct storage api's can be used.";var docProps=['title','tags','owner','readers','writers','created','modified'];function Client(app){if(app==undefined){this.log(noAppMessage,{level:'warn'});app={};}
this.app=app;this.storage=new storage.Storage(this);this.meta={};this.metaDoc={};this.metaDialog={};if(typeof $!='function'||$!=jQuery){this.onError('jQuery_required',jQueryMessage);return;}
this.appHost=window.location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.state='init';this.username=undefined;this.fLogging=true;this.logged={};this.lastHash='';this.fFirstPoll=true;this.saveInterval=60;setInterval(this.poll.fnMethod(this),ns.pollInterval);$(window).unload(this.beforeUnload.fnMethod(this));}
Client.methods({getDocURL:function(blobid){if(this.docid==undefined){return undefined;}
return this.storage.getDocURL(this.docid,blobid);},load:function(docid){if(this.app.setDoc==undefined){this.log(noSetDocMessage,{level:'warn'});return;}
if(this.isDirty()){if(!this.confirmDiscard()){return;}
this.changeState('clean');}
this.stateSave=this.state;this.changeState('loading');var self=this;this.storage.getDoc(docid,function(doc){self.setDoc(doc);self.setCleanDoc(docid);});},save:function(json,docid){if(this.isSaved()){return;}
if(json==undefined){json=this.getDoc();}
docid=docid||this.docid;if(docid==undefined){docid=this.username+'-'+base.randomInt(10000);docid=format.slugify(docid);}
this.stateSave=this.state;this.changeState('saving');var self=this;this.storage.putDoc(docid,json,function(result){result.docid=docid;result.owner=json.owner||this.username;self.onSaveSuccess(result);});},onSaveSuccess:function(result){base.extendIfChanged(this.meta,this.metaDoc,base.project(result,['modified','owner']));this.setCleanDoc(result.docid);this.setAppPanelValues(this.meta);if(this.app.onSaveSuccess){this.app.onSaveSuccess(result);}},detach:function(){this.meta.owner=this.metaDoc.owner=undefined;this.meta.modified=this.metaDoc.modified=undefined;this.setCleanDoc();this.setDirty();this.setAppPanelValues(this.meta);},getDoc:function(){var doc=typeof this.app.getDoc=='function'&&this.app.getDoc();if(typeof doc!='object'){this.log(noGetDocMessage,{level:'warn',once:true});doc={};}
base.extendIfMissing(doc,{'title':document.title});var fDoc=base.extendIfChanged(this.meta,this.metaDoc,base.project(doc,docProps));base.extendIfChanged(this.meta,this.metaDialog,this.getAppPanelValues());base.extendObject(doc,this.meta);if(fDoc){this.setAppPanelValues(this.meta);}
return doc;},setDoc:function(doc){this.meta=base.project(doc,docProps);this.setAppPanelValues(this.meta);this.app.setDoc(doc);},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=storage.jsonToString(this.getDoc());if(preserveHash){this.lastHash='';return;}
if(docid==undefined){docid='';}
this.log('clean doc: '+docid);window.location.hash=docid;this.lastHash=window.location.hash;},checkDoc:function(){if(this.saveInterval==0){return;}
...
if(this.appBar){if(this.isSaved()&&this.canSave()){$('#pfSave').addClass('disabled');}
else{$('#pfSave').removeClass('disabled');}}},beforeUnload:function(evt){if(this.state!='clean'){evt.returnValue=unloadMessage;return evt.returnValue;}},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,options){if(!this.fLogging){return;}
...
if(options.hasOwnProperty('obj')){console[options.level](message,options.obj);}else{console[options.level](message);}},errorHandler:function(xmlhttp,textStatus,errorThrown){this.changeState(this.stateSave);var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;if(message.toLowerCase()=="forbidden"){message="You don't have permission to save to this "+"document.  You may want to make a copy, instead.";}
this.log(message+' ('+code+')',{'obj':xmlhttp});this.onError(code,message);},onError:function(status,message){var formatted="client error: "+message+' ('+status+')';this.log(formatted);this.showError(message);if(this.app.onError){this.app.onError(status,message);}},poll:function(){if(this.state=='init'){if(this.getDoc){this.setCleanDoc(undefined,true);}}
if(this.lastHash!=window.location.hash){this.lastHash=window.location.hash;this.load(window.location.hash.substr(1));}
...
return;}
if(this.username||this.fFirstPoll){this.username=undefined;this.onUserChange(this.username);}},onUserChange:function(){this.log("onUserChange: "+this.username);this.updateAppBar();if(this.app.onUserChange){this.app.onUserChange(this.username);}},updateAppBar:function(){if(this.appBar){var isSignedIn=this.username!=undefined;if(isSignedIn){$('#pfWelcome').show();$('#pfUsername').text(isSignedIn?this.username:'anonymous').show();}else{$('#pfWelcome').hide();$('#pfUsername').hide();}
$('#pfSignIn').text(isSignedIn?'Sign Out':'Sign In');}},addAppBar:function(){var htmlAppBar='<div id="pfAppBarBox">'+'<div class="pfLeft"></div>'+'<div class="pfCenter">'+'<span id="pfWelcome">Welcome,</span>'+'<span class="pfLink" id="pfUsername"></span>'+'<span class="pfLink" id="pfSignIn">Sign In</span>'+'<span class="pfLink" id="pfSave">Save</span>'+'<div class="expander collapsed" id="pfMore"></div>'+'<div id="pfLogo"></div>'+'</div>'+'<div class="pfRight"></div>'+'</div>';this.appBar=document.getElementById('pfAppBar');if(!this.appBar){document.body.style.marginTop="39px";this.appBar=document.createElement('div');this.appBar.setAttribute('id','pfAppBar');this.appBar.style.position='absolute';this.appBar.style.top='0';this.appBar.style.left='0';document.body.appendChild(this.appBar);}
this.appBar.innerHTML=htmlAppBar;var self=this;$('#pfSignIn').click(function(){self.signInOut();});function onSaveClose(){self.save();self.toggleAppPanel(false);}
function onSave(){if(self.docid==undefined){self.toggleAppPanel(true);return;}
onSaveClose();}
function onCopy(){self.detach();self.toggleAppPanel();}
$('#pfSave').click(onSave);self.appPanel=document.createElement('div');self.appPanel.setAttribute('id','pfAppPanel');self.appDialog=new dialog.Dialog({fields:[{name:'message',type:'message'},{name:'title',required:true},{name:'tags'},{name:'publicReader',label:"Public",type:'checkbox'},{name:'owner',type:'value'},{name:'writers',label:"Co-authors"},{name:'modified',label:"Last Saved",type:'value'},{name:'save',label:"Save Now",type:'button',onClick:onSaveClose},{name:'copy',label:"Make a Copy",type:'button',onClick:onCopy}]});document.body.appendChild(self.appPanel);$(self.appPanel).html(self.appDialog.html());self.errorPanel=document.createElement('div');self.errorPanel.setAttribute('id','pfErrorPanel');self.errorDialog=new dialog.Dialog({fields:[{name:'error',type:'message'}]});document.body.appendChild(self.errorPanel);$(self.errorPanel).html(self.errorDialog.html());$('#pfMore').click(function(){if(self.toggleAppPanel()){self.setAppPanelValues(self.meta);}});$('#pfUsername').click(function(){window.open('http://'+self.wwwHost+'/docs/');});$('#pfLogo').click(function(){window.open('http://'+self.wwwHost);});$(window).resize(function(){self.positionAppPanel();});this.updateAppBar();},toggleAppPanel:function(fOpen){if(fOpen!=undefined&&fOpen==$(this.appPanel).is(':visible')){return;}
var self=this;$('#pfMore').toggleClass("expanded collapsed");if($(this.appPanel).is(':visible')){this.positionAppPanel('hide');return false;}else{this.positionAppPanel('show',function(){self.appDialog.setFocus();});return true;}},positionAppPanel:function(animation,fnCallback){if(animation==undefined&&!$(this.appPanel).is(':visible')){return;}
var rcAppBox=dom.getRect($('#pfAppBarBox')[0]);dom.slide(this.appPanel,vector.lr(rcAppBox),animation,fnCallback);},showError:function(message){if(this.errorPanel==undefined){return;}
var rcAppBox=dom.getRect($('#pfAppBarBox')[0]);if(message==undefined){dom.slide(this.errorPanel,vector.lr(rcAppBox),'hide');return;}
this.errorDialog.setValues({'error':message});dom.slide(this.errorPanel,vector.lr(rcAppBox),'show');var self=this;function retract(){self.showError();}
setTimeout(retract,3000);},setAppPanelValues:function(doc){if(this.appPanel==undefined){return;}
var values={};values.title=doc.title;values.owner=doc.owner;values.modified=format.shortDate(format.decodeClass(doc.modified));values.tags=format.wordList(doc.tags);values.writers=format.wordList(doc.writers);values.publicReader=base.valueInArray('public',doc.readers);if(this.docid==undefined){this.appDialog.enableField('message',true);values.message="Before saving, you can choose a new "+"title for your document.";}else{this.appDialog.enableField('message',false);}
this.appDialog.setValues(values);this.appDialog.enableField('copy',this.docid!=undefined);},getAppPanelValues:function(){if(this.appPanel==undefined||!$(this.appPanel).is(':visible')){return{};}
var values={};var dlg=this.appDialog.getValues();values.title=dlg.title;values.owner=dlg.owner;values.tags=format.arrayFromWordList(dlg.tags);values.writers=format.arrayFromWordList(dlg.writers);values.readers=dlg.publicReader?['public']:[];return values;},signInOut:function(){var isSignedIn=this.username!=undefined;if(isSignedIn){this.signOut();}
else{this.signIn();}},signIn:function(){window.open('http://'+this.wwwHost+'/sign-in/'+
this.appid+'/','_blank');},signOut:function(){cookies.setCookie('sessionuser','expired',-1);cookies.setCookie('sessionkey','expired',-1);$.ajax({dataType:'text',url:'http://'+this.appHost+'/auth/set-session/expired/',error:this.errorHandler.fnMethod(this),success:function(sessionKey,textStatus,xmlhttp){this.log("sessionkey deleted");}.fnMethod(this)});}});ns.extend({Client:Client});});
