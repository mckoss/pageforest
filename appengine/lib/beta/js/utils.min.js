...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),json);$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},putBlob:function(blobKey,data,encoding,docid){if(this.docid==undefined){this.errorReport('unsaved_document',docUnsavedMessage);return;}
var url=this.getDocURL()+blobKey;if(encoding){url+='?transfer-encoding='+encoding;}
this.log('saving blob: '+blobKey+' ('+data.length+')');$.ajax({type:'PUT',url:url,data:data,dataType:'json',processData:false,error:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.log(message+' ('+code+')',{'obj':xmlhttp});this.errorReport(code,message);}.fnMethod(this),success:function(data){this.log('saved blob: '+blobKey);if(this.app.onBlobSaveSuccess){this.app.onBlobSaveSuccess();}}.fnMethod(this)});},detach:function(){this.setCleanDoc();this.setDirty();},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=JSON.stringify(this.app.getDoc());if(preserveHash){this.lastHash='';return;}
...
return'http://'+this.appHost+'/docs/'+docid+'/';},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,options){if(!this.fLogging){return;}
