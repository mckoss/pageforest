...
return doc;},setDoc:function(doc){this.meta=base.project(doc,docProps);this.setAppPanelValues(this.meta);this.app.setDoc(doc);this.setCleanDoc(doc.doc_id);},onAutoLoad:function(message){if(!this.autoLoad||message.key!=this.docid+'/'){this.log(autoLoadError+message.key);return;}
this.load(this.docid);},setCleanDoc:function(docid,preserveHash){this.docid=docid;this.changeState('clean');this.lastJSON=storage.jsonToString(this.getDoc());if(this.autoLoad&&this.docid!=undefined){this.storage.subscribe(this.docid,undefined,{exclusive:true},this.onAutoLoad.fnMethod(this));}
