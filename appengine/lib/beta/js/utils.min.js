...
return res;}
ns.extend({extendObject:util.extendObject,Enum:Enum,StBuf:StBuf,'extendIfMissing':extendIfMissing,'extendDeep':extendDeep,'randomInt':randomInt,'strip':strip,'project':project,'uniqueArray':uniqueArray,'map':map,'filter':filter,'reduce':reduce,'keys':keys});});
...
if(options.headOnly){type='HEAD';console.log(type);}
...
else{var formatted="client error: "+message+' ('+status+')';console.log(formatted);}},poll:function(){if(this.lastHash!=window.location.hash){this.lastHash=window.location.hash;this.load(window.location.hash.substr(1));}
this.checkUsername();this.checkDoc();this.fFirstPoll=false;},checkUsername:function(){var sessionUser=cookies.getCookie('sessionuser');var usernameLast=this.username;if(sessionUser!=undefined){if(sessionUser!=this.username){this.username=sessionUser;this.log('signed in as '+this.username);if(this.app.onUserChange){this.app.onUserChange(this.username);}}}else{if(this.username||this.fFirstPoll){this.username=undefined;if(this.app.onUserChange){this.app.onUserChange(this.username);}}}},signIn:function(){window.open('http://'+this.wwwHost+'/sign-in/'+
