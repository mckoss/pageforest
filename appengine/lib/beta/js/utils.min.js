...
1000*result.lifetime;self.channel=new goog.appengine.Channel(result.token);self.socket=self.channel.open();self.socket.onmessage=self.onChannel.fnMethod();self.channelInfo=result;fnSuccess(result);}});},onChannel:function(evt){var message=evt.data;console.log("onChannel: ",message);var sub=this.subscriptions[message.key];if(sub==undefined){console.log("No subscription for channel message.",message);return;}
...
var key=docid+'/'+(blobid||'');var sub;if(fn==undefined||options&&options.enabled===false){sub=this.subscriptions[key];if(sub==undefined){return;}
