...
docid+'/'+blobid;},errorHandler:function(xmlhttp,textStatus,errorThrown){var code='ajax_error/'+xmlhttp.status;var message=xmlhttp.statusText;this.client.log(message+' ('+code+')',{'obj':xmlhttp});this.client.onError(code,message);},validateArgs:function(funcName,docid,blobid,json,options,fnSuccess){var validations={'no_username':[this.client.username,signInMessage],'missing_document_name':[docid,docidMessage],'bad_options':[typeof options!='function',badAPIMessage],'bad_callback':[fnSuccess==undefined||typeof fnSuccess=='function',badAPIMessage]};if(funcName=='putDoc'){util.extendObject(validations,{'missing_title':[typeof json=='object'||json.title,titleMessage],'missing_blob':[typeof json=='object'||json.blob,blobMessage]});}
if(funcName=='putDoc'||funcName=='putBlob'){util.extendObject(validations,{'missing_object':[json!=undefined,objectMessage]});}
...
fnSuccess=fnSuccess||function(){};options=options||{};var type='GET';if(docid==undefined){this.client.onError('unsaved_document',docUnsavedMessage);fnSuccess(false);return;}
var url=this.getDocURL(docid)+blobid;if(options.encoding){url+='?transfer-encoding='+options.encoding;}
if(options.headOnly){type='HEAD';}
this.client.log('getBlob: '+docid+'/'+blobid);$.ajax({type:type,url:url,dataType:options.dataType||'json',error:this.errorHandler.fnMethod(this),success:function(result,textStatus,xmlhttp){fnSuccess(result,textStatus,xmlhttp);}});}});ns.extend({'Storage':Storage,'jsonToString':jsonToString});});
...
Client.methods({getDocURL:function(blobid){return this.storage.getDocURL(this.docid,blobid);},load:function(docid){if(this.app.setDoc==undefined){this.log(noSetDocMessage,{level:'warn'});return;}
