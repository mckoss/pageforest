...
var data=JSON.stringify(json);this.log('saving: '+this.getDocURL(docid),{'obj':json});$.ajax({type:'PUT',url:this.getDocURL(docid),data:data,error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},putBlob:function(blobKey,data,encoding){if(this.docid==undefined){this.errorReport('unsaved_document',docUnsavedMessage);return;}
