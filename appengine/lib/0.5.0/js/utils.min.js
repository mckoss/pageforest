...
this.stateSave=this.state;this.state=Client.states.loading;this.log("loading: "+docid);$.ajax({dataType:'json',url:'http://'+this.appHost+'/docs/'+docid,error:this.errorHandler.fnMethod(this),success:function(document,textStatus,xmlhttp){this.setCleanDoc(docid);this.app.loaded(document);}.fnMethod(this)});},save:function(json,docid){if(this.username==undefined){this.errorReport('no_username',"You must sign in to save.");}
if(json==undefined){json=this.app.getData();}
...
var data=JSON.stringify(json);this.log('saving: '+docid,json);$.ajax({type:'PUT',url:'/docs/'+docid,data:data,error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.saved){this.app.saved();}}.fnMethod(this)});},setCleanDoc:function(docid){this.docid=docid;this.state=Client.states.clean;location.hash=this.docid;this.lastHash=location.hash;},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,obj){if(this.fLogging){if(obj!=undefined){console.log(message,obj);}
...
if(this.app.userChanged&&usernameLast!=this.username){this.log('found user: '+this.username);this.app.userChanged(this.username||'anonymous');}},signIn:function(){window.open('http://'+this.wwwHost+"/sign-in/"+
