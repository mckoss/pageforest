...
namespace.lookup('com.pageforest.client').defineOnce(function(ns){var cookies=namespace.lookup('org.startpad.cookies');var base=namespace.lookup('org.startpad.base');var format=namespace.lookup('org.startpad.format');var pollInterval=1000;var discardMessage="You will lose your document changes if you continue.";function Client(app){this.app=app;this.appHost=location.host;var dot=this.appHost.indexOf('.');this.appid=this.appHost.substr(0,dot);this.wwwHost='www'+this.appHost.substr(dot);this.state=Client.states.clean;this.username=undefined;this.fLogging=false;this.saveInterval=60;this.setCleanDoc();setInterval(this.poll.fnMethod(this),pollInterval);$(window).bind('beforeunload',this.beforeUnload.fnMethod(this));}
...
this.stateSave=this.state;this.changeState(Client.states.loading);this.log("loading: "+docid);$.ajax({dataType:'json',url:this.getDocURL(docid),error:this.errorHandler.fnMethod(this),success:function(document,textStatus,xmlhttp){this.app.setData(document);this.setCleanDoc(docid);}.fnMethod(this)});},getDocURL:function(docid){if(docid==undefined){docid=this.docid;}
return'http://'+this.appHost+'/docs/'+docid+'/';},save:function(json,docid){if(this.username==undefined){this.errorReport('no_username',"You must sign in to save.");return;}
...
var data=JSON.stringify(json);this.log('saving: '+docid,json);$.ajax({type:'PUT',url:'/docs/'+docid,data:data,error:this.errorHandler.fnMethod(this),success:function(data){this.setCleanDoc(docid);this.log('saved');if(this.app.onSaveSuccess){this.app.onSaveSuccess();}}.fnMethod(this)});},setCleanDoc:function(docid){this.docid=docid;this.changeState(Client.states.clean);this.lastJSON=JSON.stringify(this.app.getData());if(docid==undefined){this.lastHash='';return;}
location.hash=docid;this.lastHash=location.hash;location.href=location.href;},setLogging:function(f){f=(f==undefined)?true:f;this.fLogging=f;},log:function(message,obj){if(this.fLogging){if(obj!=undefined){console.log(message,obj);}
