...
/* Begin file: base.js */
namespace.lookup('org.startpad.base').defineOnce(function(ns){var util=namespace.util;ns.extend({extendObject:util.extendObject,extendIfMissing:function(oDest,var_args){if(oDest==undefined){oDest={};}
...
else{dest[prop]=src[prop];}}}}},randomInt:function(n){return Math.floor(Math.random()*n);},strip:function(s){return(s||"").replace(/^\s+|\s+$/g,"");},Enum:function(args){var j=0;for(var i=0;i<arguments.length;i++){if(typeof arguments[i]=="string"){this[arguments[i]]=j++;}
else{j=arguments[i];}}},project:function(obj,asProps){var objT={};for(var i=0;i<asProps.length;i++){objT[asProps[i]]=obj[asProps[i]];}
...
return res;}});ns.StBuf=function(){this.rgst=[];this.append.apply(this,arguments);};util.extendObject(ns.StBuf.prototype,{append:function(){for(var ist=0;ist<arguments.length;ist++){this.rgst.push(arguments[ist].toString());}
return this;},clear:function(){this.rgst=[];},toString:function(){return this.rgst.join("");}});});
/* Begin file: misc.js */
...
namespace.lookup('com.pageforest.client').defineOnce(function(ns){var cookies=namespace.lookup('com.pageforest.cookies');var base=namespace.lookup('org.startpad.base');var pollInterval=500;var states=new base.Enum('clean','dirty','loading','saving');function Client(app){this.app=app;var dot=location.host.indexOf('.');this.appid=location.host.substr(0,dot);this.wwwHost='www'+location.host.substr(dot);this.lastHash='';this.state=states.clean;this.username=undefined;var www="www"+location.host.substr(dot);setInterval(this.poll.fnMethod(this),pollInterval);$(window).bind('beforeunload',this.beforeUnload.fnMethod(this));}
Client.methods({load:function(docid){this.state=states.loading;this.docid=docid;$.ajax({dataType:'json',url:this.host+'/docs/'+docid,error:this.errorHandler.fnMethod(this),success:function(document,textStatus,xmlhttp){this.state=states.idle;this.app.loaded(document);}.fnMethod(this)});},save:function(json,docid){if(docid!=undefined){this.docid=docid;}
this.state=states.saving;if(!json.readers){json.readers=['public'];}
var data=JSON.stringify(json);$.ajax({type:'PUT',url:'/docs/'+this.docid,data:data,error:this.errorHandler.fnMethod(this),success:function(data){location.hash=this.docid;this.state=states.clean;if(this.app.saved){this.app.saved();}}.fnMethod(this)});},errorHandler:function(xmlhttp,textStatus,errorThrown){if(this.app.error){this.app.error(textStatus);}
else{alert(textStatus);}},makeDirty:function(fDirty){if(fDirty==undefined){fDirty=true;}
this.state=fDirty?states.dirty:states.clean;},beforeUnload:function(evt){if(this.state!=states.clean){evt.returnValue="You will lose your changes if you leave "+"the window without saving.";return evt.returnValue;}},poll:function(){if(this.lastHash!=location.hash){this.lastHash=location.hash;this.load(location.hash.substr(1));}
...
if(this.app.userChanged&&usernameLast!=this.username){this.app.userChanged(this.username||'anonymous');}},signIn:function(){window.open(this.wwwHost+"/sign-in/scratch/",'_blank');},signOut:function(){document.cookie='sessionkey=expired; path=/; expires='+'Sat, 01 Jan 2000 00:00:00 GMT';}});ns.extend({Client:Client});});
