...
global_namespace.define("com.googlecode.crypto-js",function(){});(function(){var upper='ABCDEFGHIJKLMNOPQRSTUVWXYZ';var lower='abcdefghijklmnopqrstuvwxyz';var digits='0123456789';var base64map=upper+lower+digits+'+/';var base64url=upper+lower+digits+'-_';var Crypto=global_namespace.lookup("com.googlecode.crypto-js");Crypto.base64map=base64map;Crypto.base64url=base64url;var util=Crypto.util={rotl:function(n,b){return(n<<b)|(n>>>(32-b));},rotr:function(n,b){return(n<<(32-b))|(n>>>b);},endian:function(n){if(n.constructor==Number){return util.rotl(n,8)&0x00FF00FF|util.rotl(n,24)&0xFF00FF00;}
...
ns.previous=oneline;$.ajax({type:"POST",url:"",data:data,dataType:"json",success:validate_success,error:validate_error});}
...
global_namespace.define('com.pageforest.auth.sign-in-form',function(ns){ns.extend(ns,{documentReady:function(username,appId){ns.appId=appId;ns.appDomain=ns.getAppDomain(appId);if(!username)
return;if(!appId){ns.closeForm();return;}
ns.getString(ns.appDomain+"/auth/username",function(){ns.closeForm();});},transferSession:function(sessionKey){ns.getString(ns.appDomain+"/auth/session/"+sessionKey,function(){ns.closeForm();});},getAppDomain:function(appId){var parts=window.location.host.split('.');if(parts[0]=='www')
parts[0]=appId;else
parts.splice(0,0,appId);return parts.join('.');},getString:function(url,fn){url="http://"+url;$.ajax({type:"GET",url:url,dataType:"jsonp",success:fn});},closeForm:function(){function closeFinal(){window.close();}
if(ns.appId)
$(".have_app").show('slow');$(".want_app").hide('slow');setTimeout(closeFinal,5000);}});});
