...
function document_ready(){ns.previous='~~~';setInterval(validate_if_changed,3000);$("#id_tos").click(function(){$("#validate_tos").html('');});}
function resend(){console.log("resend");$.ajax({type:"POST",url:"/email-verify/",data:{resend:true},dataType:"json",success:validate_success,error:validate_error});}
ns.extend({document_ready:document_ready,resend:resend});});
/* Begin file: sign-in-form.js */
namespace.lookup('com.pageforest.auth.sign-in-form').define(function(ns){var cookies=namespace.lookup('org.startpad.cookies');ns.extend({documentReady:function(username,appId){ns.appId=appId;ns.appAuthURL=ns.getAppDomain(appId)+'/auth/';var sessionName=appId+"-sessionkey";var appSession=cookies.getCookies()[sessionName];console.log("appSession: ",appSession);if(appSession!=undefined){cookies.setCookie(sessionName,'expired',-1);ns.transferSession(appSession);}
if(!username){return;}
if(!appId){ns.closeForm();return;}
ns.getString(ns.appAuthURL+"username/",function(username){if(typeof(username)=='string'){ns.closeForm();return;}});},transferSession:function(sessionKey){var url=ns.appAuthURL+"set-session/"+sessionKey;ns.getString(url,function(s){if(typeof(s)!='string'){return;}
if(sessionKey){ns.closeForm();}});},getAppDomain:function(appId){var parts=window.location.host.split('.');if(parts[0]=='www'){parts[0]=appId;}
else{parts.splice(0,0,appId);}
return parts.join('.');},getString:function(url,fn){url="http://"+url;$.ajax({type:"GET",url:url,dataType:"jsonp",success:fn,error:function(){fn({status:500});}});},closeForm:function(){function closeFinal(){window.close();}
if(ns.appId){$(".have_app").show();}
$(".want_app").hide();setTimeout(closeFinal,2000);}});});
