...
namespace.lookup('com.pageforest.auth.sign-in').define(function(ns){var main=namespace.lookup('com.pageforest.main');var cookies=namespace.lookup('org.startpad.cookies');var crypto=namespace.lookup('com.googlecode.crypto-js');var forms=namespace.lookup('com.pageforest.forms');var dialog=namespace.lookup('org.startpad.dialog');var appId;var appAuthURL;var sessionKey;var dlg;function getAppDomain(appId){var parts=window.location.host.split('.');if(parts[0]=='www'){parts[0]=appId;}else{parts.splice(0,0,appId);}
return parts.join('.');}
...
function onSuccess(message,status,xhr){$(document.body).addClass('user');$('.username').text(dlg.values.username);getSessionKey(function(){if(dlg.values.allowAccess){transferSessionKey(closeForm);}});}
