...
namespace.lookup('com.pageforest.registration').define(function(ns){var util=namespace.util;var Crypto=namespace.lookup('com.googlecode.crypto-js');function onValidateSuccess(message,status,xhr,options){var force=options&&options.force;var fields=["username","password","email","tos"];for(var index=0;index<fields.length;index++){var name=fields[index];var html=message[name];if(!force&&$("#id_"+name).val()===''){html='';}else if(html){html='<span class="error">'+html+'</span>';}else{html='<span class="success">OK</span>';}
...
function validateIfChanged(){var data=getFormData();data.validate=true;var oneline=[data.username,data.email].join('|');if(ns.previous!=oneline){ns.previous=oneline;postFormData(data,onValidateSuccess,onError);}}
function onSubmit(){var data=getFormData();postFormData(data,onSubmitSuccess,onError);return false;}
...
function onReady(){ns.previous='|';setInterval(validateIfChanged,1000);$("#id_tos").click(function(){$("#validate_tos").html('');});}
ns.extend({onReady:onReady,onSubmit:onSubmit,resend:resend});});
/* Begin file: sign-in-form.js */
namespace.lookup('com.pageforest.auth.sign-in-form').define(function(ns){var cookies=namespace.lookup('org.startpad.cookies');function getAppDomain(appId){var parts=window.location.host.split('.');if(parts[0]=='www'){parts[0]=appId;}else{parts.splice(0,0,appId);}
return parts.join('.');}
...
ns.extend({transferSession:transferSession,onReady:onReady});});
