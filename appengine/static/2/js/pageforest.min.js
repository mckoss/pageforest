...
function transferSession(fn){var url=ns.appAuthURL+"set-session/"+ns.sessionKey;getJSONP(url,function(message){if(typeof(message)!='string'){return;}
if(fn){fn();}});return false;}
function onSuccess(message,status,xhr){ns.sessionKey=message;transferSession(function(){window.location.reload();});}
...
function onReady(username,appId,sessionKey){$('#enablejs').hide();$('input').removeAttr('disabled');if(cookies.getCookie('httponly')){$('#httponly').show();}
ns.appId=appId;ns.sessionKey=sessionKey;ns.appAuthURL='http://'+getAppDomain(appId)+'/auth/';if(!username){return;}
var url=ns.appAuthURL+"username/";getJSONP(url,function(username){if(typeof(username)=='string'){closeForm();return;}});}
function signOut(){if(ns.appId){ns.sessionKey='expired';transferSession(function(){window.location='/sign-out/'+ns.appId;});return;}
window.location='/sign-out/';}
ns.extend({'onReady':onReady,'onSubmit':onSubmit,'transferSession':transferSession,'signOut':signOut,'closeForm':closeForm});});
