...
function onSuccess(message,status,xhr){window.location.reload();}
function onValidate(message,status,xhr){forms.showValidatorResults(['username','password'],message);}
function onError(xhr,status,message){console.error(status+': '+message);}
function onSubmit(){var username=$('#id_username').val();var password=$('#id_password').val();var data={username:username,password:crypto.HMAC(crypto.SHA1,username,password)};forms.postFormData('/sign-in/',data,onSuccess,onValidate,onError);return false;}
