...
function onSuccess(message,status,xhr){$(document.body).addClass('user');$('.username').text(username);getSessionKey(function(){if($('#id-appauth').attr('checked')){transferSessionKey(closeForm);}});}
...
function onChallenge(challenge,status,xhr){username=$('#id-username').val();var lower=username.toLowerCase();var password=$('#id-password').val();var userpass=crypto.HMAC(crypto.SHA1,lower,password);var signature=crypto.HMAC(crypto.SHA1,challenge,userpass);var reply=lower+'|'+challenge+'|'+signature;$.ajax({url:'/auth/verify/'+reply,success:onSuccess,error:onError});}
function onSubmit(){$.ajax({url:'/auth/challenge',success:onChallenge,error:onError});return false;}
function onReady(forApp){username=cookies.getCookie('sessionuser');appId=forApp;if(appId){appAuthURL='http://'+getAppDomain(appId)+'/auth/';}
$(document.body)[username?'addClass':'removeClass']('user');$(document.body)[appId?'addClass':'removeClass']('app');if(username){$('.username').text(username);getSessionKey();}
if(appId){var url=appAuthURL+"username/";getJSONP(url,function(username){if(typeof(username)=='string'){closeForm();return;}});}}
