...
namespace.lookup('com.pageforest.auth.sign-up').define(function(ns){var util=namespace.util;var cookies=namespace.lookup('org.startpad.cookies');var crypto=namespace.lookup('com.googlecode.crypto-js');function validatePassword(){var password=$("#id_password").val();var repeat=$("#id_repeat").val();if(!password.length){return{password:"This field is required."};}
...
function onReady(){$('#enablejs').hide();if(cookies.getCookie('httponly')){$('#httponly').show();}
...
function onReady(username,appId){$('#enablejs').hide();if(cookies.getCookie('httponly')){$('#httponly').show();}
ns.appId=appId;ns.appAuthURL='http://'+getAppDomain(appId)+'/auth/';var sessionName=appId+"-sessionkey";var appSession=cookies.getCookie(sessionName);console.log("appSession: ",appSession);if(appSession!=undefined){cookies.setCookie(sessionName,'expired',-1);transferSession(appSession);}
if(!username){return;}
