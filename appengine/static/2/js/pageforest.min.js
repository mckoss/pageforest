...
$(document.body).addClass('session');sessionKey=json.sessionKey;});}
...
function onSuccess(message,status,xhr){$(document.body).addClass('user');$('.username').text(username);getSessionKey(transferSessionKey);}
...
function onChallenge(challenge,status,xhr){username=$('#id_username').val();var lower=username.toLowerCase();var password=$('#id_password').val();var userpass=crypto.HMAC(crypto.SHA1,lower,password);var signature=crypto.HMAC(crypto.SHA1,challenge,userpass);var reply=lower+'|'+challenge+'|'+signature;$.ajax({url:'/auth/verify/'+reply,success:onSuccess,error:onError});}
function onSubmit(){$.ajax({url:'/auth/challenge',success:onChallenge,error:onError});return false;}
function onReady(usernameT,appIdT){appId=appIdT;username=usernameT;if(appId){appAuthURL='http://'+getAppDomain(appId)+'/auth/';}
