...
namespace.lookup('com.pageforest.auth.sign-up').define(function(ns){var cookies=namespace.lookup('org.startpad.cookies');var crypto=namespace.lookup('com.googlecode.crypto-js');var forms=namespace.lookup('com.pageforest.forms');var dialog=namespace.lookup('org.startpad.dialog');var dlg;function validatePassword(){var password=$("#id_password").val();var repeat=$("#id_repeat").val();if(!password.length){return{password:"This field is required."};}
...
function validateIfChanged(){if(!dlg.hasChanged(undefined,true)){return;}
var data=dlg.getValues();data.validate=true;forms.postFormData('/sign-up/',data,null,onValidateIgnoreEmpty,onError);}
function onSubmit(){var errors=validatePassword();if(errors){forms.showValidatorResults(['password','repeat'],errors);}else{forms.postFormData('/sign-up/',dlg.getValues(),onSuccess,onValidate,onError);}
return false;}
...
function onReady(){dlg=new dialog.Dialog({fields:[{name:'username'},{name:'password',type:'password'},{name:'passwordRepeat',type:'password',label:"Repeat password"},{name:'email',label:"Email address"},{name:'tos',type:'checkbox',label:"Terms of Service"},{name:'joinNow',label:"Join Now",type:'button',onClick:onSubmit}],style:dialog.styles.table});$('#sign-up-dialog').html(dlg.html());dlg.setFocus();setInterval(validateIfChanged,1000);}
ns.extend({onReady:onReady,onSubmit:onSubmit,resend:resend});});
/* Begin file: sign-in.js */
namespace.lookup('com.pageforest.auth.sign-in').define(function(ns){var main=namespace.lookup('com.pageforest.main');var cookies=namespace.lookup('org.startpad.cookies');var crypto=namespace.lookup('com.googlecode.crypto-js');var forms=namespace.lookup('com.pageforest.forms');var dialog=namespace.lookup('org.startpad.dialog');var format=namespace.lookup('org.startpad.format');var appId;var appAuthURL;var sessionKey;var dlg;function getAppDomain(appId){var parts=window.location.host.split('.');if(parts[0]=='www'){parts[0]=appId;}else{parts.splice(0,0,appId);}
return parts.join('.');}
...
var params=format.parseURLParams(window.location.href);if(params['continue']){window.location=params['continue'];return;}});}
...
function onReady(forApp){var username=cookies.getCookie('sessionuser');appId=forApp;dlg=new dialog.Dialog({fields:[{name:'username'},{name:'password',type:'password'},{name:'allowAccess',label:"Allow Access to "+appId,type:'checkbox'},{name:'signIn',label:"Sign In",type:'button',onClick:onSubmit}],style:dialog.styles.table});$('#sign-in-dialog').html(dlg.html());dlg.setFocus();if(appId){appAuthURL='http://'+getAppDomain(appId)+'/auth/';}else{dlg.showField('allowAccess',false);}
...
ns.extend({'onReady':onReady,'onSubmit':onSubmit,'transferSessionKey':transferSessionKey,'signOut':signOut,'closeForm':closeForm});});
